'''
程序说明：
## 1. 本程序为3.1.5版股票历史数据查询助手，基于Qwen_agent的Assistant，支持终端TUI和Web GUI两种模式。
## 2. 在3.1.4基础上，新增Boll布林带异常点检测功能：工具boll_detection支持对指定股票过去一年（或自定义区间）进行20日周期+2σ的超买超卖点检测，输出表格和图片。
## 3. K线图绘制部分用plotly，其他可视化为matplotlib，输出图片目录、文件名规则与3.1.4一致。
'''
...
class BollDetectionTool(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        try:
            ...
            plt.tight_layout()
            # 保存图片
            boll_dir = os.path.join(os.path.dirname(__file__), '3.1.5_boll_image_show')
...
class ArimaStockTool(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        try:
            ...
            plt.tight_layout()
            # 保存图片
            pred_dir = os.path.join(os.path.dirname(__file__), '3.1.5_arima_image_show')
...
class ExcSQLTool(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        try:
            ...
            # K线图输出目录
            kline_dir = os.path.join(os.path.dirname(__file__), '3.1.5_image_show')
            # 折线/对比图输出目录
            plot_dir = os.path.join(os.path.dirname(__file__), '3.1.5_image_show')
...
def init_agent_service():
    ...
    }, 'exc_sql', 'arima_stock', 'boll_detection']
    try:
        bot = Assistant(
            llm=llm_cfg,
            name='股票历史查询助手',

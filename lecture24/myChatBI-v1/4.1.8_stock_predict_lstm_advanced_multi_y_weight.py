'''
程序说明：
## 1. 本程序在4.1.7_stock_predict_lstm_advanced_multi_y_vol_2.py基础上，升级为多目标预测（open、close、high、low、vol），递推链条每步预测5个特征。
## 2. 主要改进：LSTM输出层为Dense(5)，损失函数为"多目标加权MSE"，价格目标（open、close、high、low）损失权重为1，vol损失权重为0.01。
## 3. 输出目录为4.1.8_lstm_pred_image_show，所有输出、可视化、特征导出等与4.1.7一致。
...
def main():
    ...
    save_dir = '4.1.8_lstm_pred_image_show'

'''
程序说明：
## 1. 本程序在4.1.8_stock_predict_lstm_advanced_multi_y_weight.py基础上，进一步丰富了衍生特征，所有衍生特征均可由现有特征计算得出，无需预测。
## 2. 主要改进：增加了如振幅、收盘与开盘价差、最高最低价差、量比、换手率、收盘价与均线比、成交额与均值比等多种衍生特征，提升模型表达力。
## 3. 输出目录为4.1.9_lstm_pred_image_show，模型目录为4.1.9_lstm_model，所有输出、可视化、特征导出等与4.1.8一致。
## 4. 价格（open、close、high、low）和vol分开归一化，分别用独立的MinMaxScaler，训练、递推、反归一化时保持一致。
## 5. 可视化新增预测最高价、最低价曲线，便于分析预测区间。
## 6. 新增局部K线图可视化，直观展示历史与未来预测K线。
## 7. 新增plotly版局部K线图可视化，交互体验更佳。
## 8. 其它功能、注释、输出、可视化等全部保留并兼容。
'''
...
def add_derived_features(df):
    """
    :function: 计算重要衍生特征，包括ma5、ma10、close_ma5_diff、close_ma10_diff、pct_chg_mean5、pct_chg_std5、振幅、收盘与开盘价差、最高最低价差、量比、换手率、收盘价与均线比、成交额与均值比等
    ...
    # 新增衍生特征
    df['abs_pct_chg'] = df['pct_chg'].abs()  # 涨跌幅绝对值
    df['amplitude'] = (df['high'] - df['low']) / df['pre_close'].replace(0, np.nan)  # 振幅
    df['close_open_diff'] = df['close'] - df['open']  # 收盘与开盘价差
    df['high_low_diff'] = df['high'] - df['low']      # 最高最低价差
    df['vol_ratio'] = df['vol'] / (df['vol'].rolling(5).mean().replace(0, np.nan))  # 量比
    df['turnover_rate'] = df['vol'] / (df['vol'].sum() + 1e-6)  # 换手率（近似，真实需总股本）
    df['close_ma5_ratio'] = df['close'] / (df['ma5'].replace(0, np.nan))  # 收盘价与5日均线比
    df['close_ma10_ratio'] = df['close'] / (df['ma10'].replace(0, np.nan))  # 收盘价与10日均线比
    df['amount_mean5_ratio'] = df['amount'] / (df['amount'].rolling(5).mean().replace(0, np.nan))  # 成交额与5日均值比
...
def normalize_features(X, y, price_scaler=None, vol_scaler=None, fit=True):
    """
    :function: 特征归一化，价格和vol分开归一化，分别用独立的MinMaxScaler
    :param X: 特征
    :param y: 标签（open、close、high、low、vol）
    :param price_scaler: 价格scaler
    :param vol_scaler: vol scaler
...
def weighted_multi_mse(y_true, y_pred):
    """
    :function: 多目标加权MSE损失，价格权重1，vol权重0.1
...
# ===================== 主流程 =====================
def rolling_predict_df(model, df_hist, window, n, feature_cols, stat_dict, method='future', y_true=None, dates=None, debug_log=None, debug_predict_log=None, price_scaler=None, vol_scaler=None):
    preds = []
    features = []
...
def main():
    ...
    save_dir = '4.1.9_lstm_pred_image_show'
    ...
    future_preds, future_feats = rolling_predict_df(model, df, window, pred_days, feature_cols, stat_dict, method='future', dates=pred_dates, debug_log=debug_log, debug_predict_log=debug_predict_log, price_scaler=price_scaler, vol_scaler=vol_scaler)
    val_dates = df['trade_date'].iloc[-pred_days:]
    df_val1 = df.iloc[-2*pred_days-window:-pred_days].copy().reset_index(drop=True)
    val1_preds, val1_feats = rolling_predict_df(model, df_val1, window, pred_days, feature_cols, stat_dict, method='val1', dates=val_dates, debug_log=debug_log, debug_predict_log=debug_predict_log, price_scaler=price_scaler, vol_scaler=vol_scaler)
    df_val2 = df.iloc[-2*pred_days-window:-pred_days].copy().reset_index(drop=True)
    y_true_val2 = y_seq[-pred_days:]
    val2_preds, val2_feats = rolling_predict_df(model, df_val2, window, pred_days, feature_cols, stat_dict, method='val2', y_true=y_true_val2, dates=val_dates, debug_log=debug_log, debug_predict_log=debug_predict_log, price_scaler=price_scaler, vol_scaler=vol_scaler)
...
# ========== 可视化 ==========
    ...
    # 新增：局部K线图
    def plot_zoomed_kline(df, pred_dates, future_preds, stock_name, save_dir, pred_days):
        """
        :function: 绘制局部K线图，蓝色为历史K线，红色为未来预测K线
        :param df: 历史DataFrame
        :param pred_dates: 预测日期序列
        :param future_preds: 未来预测(open, close, high, low, vol)
        ...
        idx_start = max(0, df.index[-pred_days-5])
        idx_end = len(df)
        df_zoom = df.iloc[idx_start:idx_end].copy().reset_index(drop=True)
        # 历史K线数据
        hist_dates = df_zoom['trade_date']
        hist_open = df_zoom['open']
        hist_close = df_zoom['close']
        hist_high = df_zoom['high']
        hist_low = df_zoom['low']
        # 预测K线数据
        pred_open = future_preds[:,0]
        pred_close = future_preds[:,1]
        pred_high = future_preds[:,2]
        pred_low = future_preds[:,3]
        # 合并日期
        ...
        for i in range(len(hist_dates)):
            color = 'blue' if hist_close.iloc[i] >= hist_open.iloc[i] else 'cyan'
            ax.plot([hist_dates.iloc[i], hist_dates.iloc[i]], [hist_low.iloc[i], hist_high.iloc[i]], color=color, linewidth=1)
            ax.plot([hist_dates.iloc[i], hist_dates.iloc[i]], [hist_open.iloc[i], hist_close.iloc[i]], color=color, linewidth=4)
        # 画预测K线
        for i in range(len(pred_dates)):
            color = 'red' if pred_close[i] >= pred_open[i] else 'magenta'
            ax.plot([pred_dates.iloc[i], pred_dates.iloc[i]], [pred_low[i], pred_high[i]], color=color, linewidth=1)
            ax.plot([pred_dates.iloc[i], pred_dates.iloc[i]], [pred_open[i], pred_close[i]], color=color, linewidth=4)
        ax.set_title(f'{stock_name} 局部K线图（历史:蓝/未来:红）')
        ax.set_xlabel('日期')
        ax.set_ylabel('价格')
        ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
        ax.xaxis.set_major_locator(mdates.AutoDateLocator())
        fig.autofmt_xdate()
        blue_patch = plt.Line2D([0], [0], color='blue', lw=4, label='历史K线')
        red_patch = plt.Line2D([0], [0], color='red', lw=4, label='预测K线')
        ax.legend(handles=[blue_patch, red_patch])
        filename = f'{safe_filename(stock_name)}_kline_zoomed_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png'
        save_path = os.path.join(save_dir, filename)
        plt.savefig(save_path)
        plt.close()
        print(f'局部K线图已保存：{save_path}')
    plot_zoomed_kline(df, pred_dates, future_preds, stock_name, save_dir, pred_days)
    # 新增：plotly版局部K线图
    def plot_zoomed_kline_plotly(df, pred_dates, future_preds, stock_name, save_dir, pred_days):
        """
        :function: 用plotly绘制局部K线图，历史为蓝色，预测为红色，保存为html
        :param df: 历史DataFrame
        :param pred_dates: 预测日期序列
        :param future_preds: 未来预测(open, close, high, low, vol)
        :param stock_name: 股票名
        ...
        fig.add_trace(go.Candlestick(
            ...
            low=hist_low,
            close=hist_close,
            increasing_line_color='blue', decreasing_line_color='cyan',
            name='历史K线'))
        # 预测K线
        fig.add_trace(go.Candlestick(
            x=pred_dates,
            ...
            low=pred_low,
            close=pred_close,
            increasing_line_color='red', decreasing_line_color='magenta',
            name='预测K线'))
        fig.update_layout(
            title=f'{stock_name} 局部K线图（plotly，历史:蓝/未来:红）',
            xaxis_title='日期',
            yaxis_title='价格',
            legend=dict(orientation='h', yanchor='bottom', y=1.02, xanchor='right', x=1),
            xaxis_rangeslider_visible=False
        )
        filename = f'{safe_filename(stock_name)}_kline_zoomed_plotly_{datetime.now().strftime("%Y%m%d_%H%M%S")}.html'
        save_path = os.path.join(save_dir, filename)
        fig.write_html(save_path)
        print(f'plotly局部K线图已保存：{save_path}')
    plot_zoomed_kline_plotly(df, pred_dates, future_preds, stock_name, save_dir, pred_days)
    print("\n主要特征工程说明：")

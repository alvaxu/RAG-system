'''
程序说明：
## 1. 本程序为3.1.3版股票历史数据查询助手，基于Qwen_agent的Assistant，支持终端TUI和Web GUI两种模式。
## 2. 在3.1.2基础上，K线图绘制部分用plotly替换mplfinance，生成静态K线图图片，支持中文标题、坐标轴、成交量且无乱码。
## 3. 其余可视化（如走势、对比等）仍为matplotlib折线图，标题和legend风格与3.1.2一致。
## 4. 输出图片目录、文件名规则与3.1.2一致，兼容WebUI自动渲染。
'''
...
class ExcSQLTool(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        try:
            ...
            # K线图输出目录
            kline_dir = os.path.join(os.path.dirname(__file__), '3.1.3_image_show')
            # 折线/对比图输出目录
            plot_dir = os.path.join(os.path.dirname(__file__), '3.1.3_image_show')

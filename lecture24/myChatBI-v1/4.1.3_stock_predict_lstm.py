...
def add_derived_features(df):
    ...
    # pct_chg分母保护
    df['pct_chg'] = np.where(df['pre_close'] != 0, (df['close'] - df['pre_close']) / df['pre_close'] * 100, 0)
    df['pct_chg_mean5'] = df['pct_chg'].rolling(5).mean()
    df['pct_chg_std5'] = df['pct_chg'].rolling(5).std()
    # vol_chg分母保护
    df['vol_chg'] = np.where(df['vol'].shift(1) != 0, df['vol'].pct_change(), 0)
...
def rolling_predict_df(model, df_hist, window, n, feature_cols, method='future', y_true=None, dates=None, debug_log=None):
    """
    :function: 递推式预测/验证，窗口为DataFrame，每步append新行并重算所有特征，递推链条不断，涨跌幅保护，源头杜绝nan/inf
    :param model: 已训练模型
    ...
    for i in range(n):
        ...
        # 打印递推窗口主要特征内容到debug_log
        if debug_log is not None:
            debug_log.write(f"\n递推步{i+1} 输入窗口close: {window_df['close'].values}\n")
            debug_log.write(f"递推步{i+1} 输入窗口ma5: {window_df['ma5'].values}\n")
            debug_log.write(f"递推步{i+1} 输入窗口ma10: {window_df['ma10'].values}\n")
            debug_log.write(f"递推步{i+1} 输入窗口pct_chg: {window_df['pct_chg'].values}\n")
        pred = float(model.predict(X_input[np.newaxis, :, :]).flatten()[0])
        if debug_log is not None:
            debug_log.write(f"递推步{i+1} 模型输出: {pred}\n")
        ...
        if method == 'future' or method == 'val1':
            ...
            new_row['high'] = max(pred * high_ratio, pred)
            new_row['low'] = max(pred * low_ratio, 0.01)
            prev_close = pred
        elif method == 'val2':
            if y_true is not None and i < len(y_true):
                new_row['pre_close'] = prev_close
                new_row['open'] = prev_close
                new_row['close'] = max(y_true[i], 0.01)
                high_ratio = 1 + np.random.uniform(0.01, 0.02)
                low_ratio = 1 - np.random.uniform(0.01, 0.02)
                new_row['high'] = max(y_true[i] * high_ratio, y_true[i])
                new_row['low'] = max(y_true[i] * low_ratio, 0.01)
                prev_close = max(y_true[i], 0.01)
...
def main():
    ...
    if need_train:
        ...
        model.fit(X_train, y_train, epochs=100 if not DEBUG else 5, batch_size=16, verbose=2, validation_split=0.1, callbacks=[early_stop])
        print("训练完成，保存模型...")
    ...
    future_preds, future_feats = rolling_predict_df(model, df, window, pred_days, feature_cols, method='future', dates=pred_dates, debug_log=debug_log)
    # ========== 验证集递推预测 ==========
    val_dates = df['trade_date'].iloc[-pred_days:]
    df_val1 = df.iloc[-2*pred_days-window:-pred_days].copy().reset_index(drop=True)
    val1_preds, val1_feats = rolling_predict_df(model, df_val1, window, pred_days, feature_cols, method='val1', dates=val_dates, debug_log=debug_log)
    df_val2 = df.iloc[-2*pred_days-window:-pred_days].copy().reset_index(drop=True)
    y_true_val2 = y_seq[-pred_days:]
    val2_preds, val2_feats = rolling_predict_df(model, df_val2, window, pred_days, feature_cols, method='val2', y_true=y_true_val2, dates=val_dates, debug_log=debug_log)
    debug_log.close()

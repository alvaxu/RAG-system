'''
程序说明：
## 1. 本程序在4.1.4_stock_predict_lstm_advanced.py基础上，升级为多目标预测（open、close、high、low），递推链条每步预测4个价格特征。
## 2. 主要改进：LSTM输出层为Dense(4)，损失函数为MSE，递推时窗口open/close/high/low均用模型预测值，pre_close用上一步的close。
## 3. 输出目录为4.1.5_lstm_pred_image_show，所有输出、可视化、特征导出等与4.1.4一致。
## 4. 其它功能、注释、输出、可视化等全部保留并兼容。
'''
...
def build_lstm_model(input_shape):
    ...
    model.add(Dense(4))  # 多目标输出open、close、high、low
...
def train_model(model, X_train, y_train, model_path, save_dir, stock_name, window):
    ...
    print('训练集预测分布:')
    print('open均值:', np.mean(y_pred[:,0]), 'close均值:', np.mean(y_pred[:,1]), 'high均值:', np.mean(y_pred[:,2]), 'low均值:', np.mean(y_pred[:,3]))
    print('open方差:', np.var(y_pred[:,0]), 'close方差:', np.var(y_pred[:,1]), 'high方差:', np.var(y_pred[:,2]), 'low方差:', np.var(y_pred[:,3]))
    return history
...
# ===================== 主流程 =====================
def rolling_predict_df(model, df_hist, window, n, feature_cols, stat_dict, method='future', y_true=None, dates=None, debug_log=None, debug_predict_log=None):
    preds = []
    features = []
    ...
    for i in range(n):
        ...
        if debug_predict_log is not None:
            ...
            try:
                debug_predict_log.write('输入窗口特征（open/close/high/low）：\n')
                debug_predict_log.write(str(window_df[['open','close','high','low']].values) + '\n')
            except Exception as e:
        ...
        pred = model.predict(X_input[np.newaxis, :, :]).flatten()
        pred_open, pred_close, pred_high, pred_low = float(pred[0]), float(pred[1]), float(pred[2]), float(pred[3])
        if debug_predict_log is not None:
            debug_predict_log.write(f"模型输出y_pred: {[pred_open, pred_close, pred_high, pred_low]}\n")
        if debug_log is not None:
            debug_log.write(f"递推步{i+1} 模型输出: open={pred_open}, close={pred_close}, high={pred_high}, low={pred_low}\n")
        pred_high = max(pred_high, pred_open, pred_close)
        pred_low = min(pred_low, pred_open, pred_close)
        if debug_predict_log is not None:
            try:
                debug_predict_log.write('滑动前窗口open/close/high/low：\n')
                debug_predict_log.write(str(window_df[['open','close','high','low']].values) + '\n')
            except Exception as e:
        ...
        preds.append([pred_open, pred_close, pred_high, pred_low])
        window_df = window_df.iloc[1:].reset_index(drop=True)
        if debug_predict_log is not None:
            try:
                debug_predict_log.write('滑动后窗口open/close/high/low：\n')
                debug_predict_log.write(str(window_df[['open','close','high','low']].values) + '\n')
            except Exception as e:
...
def main():
    ...
    y = df[['open','close','high','low']].values
    ...
    save_dir = '4.1.5_lstm_pred_image_show'
    ...
    future_preds, future_feats = rolling_predict_df(model, df, window, pred_days, feature_cols, stat_dict, method='future', dates=pred_dates, debug_log=debug_log, debug_predict_log=debug_predict_log)
    val_dates = df['trade_date'].iloc[-pred_days:]
    df_val1 = df.iloc[-2*pred_days-window:-pred_days].copy().reset_index(drop=True)
    val1_preds, val1_feats = rolling_predict_df(model, df_val1, window, pred_days, feature_cols, stat_dict, method='val1', dates=val_dates, debug_log=debug_log, debug_predict_log=debug_predict_log)
    df_val2 = df.iloc[-2*pred_days-window:-pred_days].copy().reset_index(drop=True)
    y_true_val2 = y_seq[-pred_days:]
    val2_preds, val2_feats = rolling_predict_df(model, df_val2, window, pred_days, feature_cols, stat_dict, method='val2', y_true=y_true_val2, dates=val_dates, debug_log=debug_log, debug_predict_log=debug_predict_log)
    debug_predict_log.close()
    print('详细调试信息见debug_log.txt')
    table_rows = []
    for i in range(pred_days):
        row_real = {'日期': val_dates.iloc[i].strftime('%Y-%m-%d'), '开盘价': y_seq[-pred_days:][i,0], '收盘价': y_seq[-pred_days:][i,1], '最高价': y_seq[-pred_days:][i,2], '最低价': y_seq[-pred_days:][i,3], '类型': '真实'}
        row_val1 = {'日期': val_dates.iloc[i].strftime('%Y-%m-%d'), '开盘价': val1_preds[i,0], '收盘价': val1_preds[i,1], '最高价': val1_preds[i,2], '最低价': val1_preds[i,3], '类型': '验证方法一'}
        row_val2 = {'日期': val_dates.iloc[i].strftime('%Y-%m-%d'), '开盘价': val2_preds[i,0], '收盘价': val2_preds[i,1], '最高价': val2_preds[i,2], '最低价': val2_preds[i,3], '类型': '验证方法二'}

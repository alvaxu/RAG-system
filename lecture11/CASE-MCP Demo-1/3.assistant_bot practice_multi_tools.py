...
主要功能：
- 通过自然语言进行地图、天气、IP、SQL等多种服务查询
- 支持多种交互方式(GUI、TUI、测试模式)
- 支持旅游规划、地点查询、路线导航、天气、IP定位、数据库等
"""
    ...
    tools = [{
        "mcpServers": {
            ...
            "txt-counter-practice": {
                "command": "python",
                "args": ["0.txt_counter_practice.py"],
                "port": 6274
            }
        }
    }, 'exc_sql', 'get_current_weather', 'get_ip_location']

    try:
        bot = Assistant(
            llm=llm_cfg,
            name='多工具AI助手',
            description='地图/网页/搜索/SQL/天气/IP定位等多功能智能助手',
    ...
    try:
        ...
        chatbot_config = {
            'prompt.suggestions': [
                ...
                '北京现在天气怎么样？',
                '114.247.50.2的IP地址在哪个城市？',
                '114.247.50.2的IP地址所在城市的天气情况怎样？'
            ]
        }

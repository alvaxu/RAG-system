...
[申购] 最优参数: {'changepoint_prior_scale': 0.01, 'seasonality_prior_scale': 20.0, 'holidays_prior_scale': 10.0, 'seasonality_mode': 'multiplicative', 'changepoint_range': 0.9, 'n_changepoints': 45}, 最优验证集RMSE: 53518340.35

[赎回] 最优参数: {'changepoint_prior_scale': 0.01, 'seasonality_prior_scale': 5.0, 'holidays_prior_scale': 20.0, 'seasonality_mode': 'additive', 'changepoint_range': 0.9, 'n_changepoints': 30}, 最优验证集RMSE: 59251499.80
- 不引入外部变量。
'''
...
# 创建输出目录
os.makedirs('3.2.2_prophet_output', exist_ok=True)
os.makedirs('3.2.2_feature_output', exist_ok=True)
...
def auto_prophet(train_df, holidays, all_dates, label, out_prefix):
    ...
    param_grid = {
        ...
        'n_changepoints': [25,30,35,40,45,50]
    }
    ...
    plt.savefig(f'3.2.2_prophet_output/3.2.2_prophet_{out_prefix}_components.png')
    plt.close()
...
def main():
    ...
    purchase_df.to_csv('3.2.2_feature_output/3.2.2_prophet_purchase.csv', index=False, encoding='utf-8-sig')
    redeem_df.to_csv('3.2.2_feature_output/3.2.2_prophet_redeem.csv', index=False, encoding='utf-8-sig')
    ...
    holiday_df.to_csv('3.2.2_feature_output/3.2.2_prophet_holidays.csv', index=False, encoding='utf-8-sig')
    ...
    daily.to_csv('3.2.2_feature_output/3.2.2_prophet_daily_features.csv', index=False, encoding='utf-8-sig')
    ...
    purchase_df = pd.read_csv('3.2.2_feature_output/3.2.2_prophet_purchase.csv', parse_dates=['ds'])
    redeem_df = pd.read_csv('3.2.2_feature_output/3.2.2_prophet_redeem.csv', parse_dates=['ds'])
    holidays = pd.read_csv('3.2.2_feature_output/3.2.2_prophet_holidays.csv', parse_dates=['ds'])
    ...
    plt.savefig('3.2.2_prophet_output/3.2.2_prophet_purchase_redeem_compare.png')
    plt.close()
    ...
    plt.savefig('3.2.2_prophet_output/3.2.2_prophet_purchase_residuals.png')
    plt.close()
    # 赎回残差
    ...
    plt.axhline(0, color='red', linestyle='--')
    plt.savefig('3.2.2_prophet_output/3.2.2_prophet_redeem_residuals.png')
    plt.close()
    ...
    predict_df.to_csv('3.2.2_prophet_output/3.2.2_prophet_predict.csv', index=False, header=False, encoding='utf-8-sig')

    # 保存最优参数和RMSE到txt文件
    with open('3.2.2_prophet_output/3.2.2_prophet_best_params.txt', 'w', encoding='utf-8') as f:
    ...
    print('Prophet自动调参建模、分解图与残差分析已完成，结果保存在3.2.2_prophet_output目录。')

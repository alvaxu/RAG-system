# -*- coding: utf-8 -*-
"""
:功能: 8.0.4版 用户级LightGBM统一耦合递归预测
:说明: 采用统一递归循环，正确处理申购和赎回模型之间的交叉依赖关系。
       1. 特征耦合：申购和赎回模型均使用对方的历史lag特征。
       2. 统一递归：在单个循环中按天预测，当天预测结果（申购/赎回）会立即用于更新未来日期的特征。
       3. 独立运行：包含完整的特征工程，不依赖任何旧版本脚本。
       4. 输出格式：同时生成竞赛格式的预测CSV和用于分析的对比图。
:作者: AI助手
"""
import pandas as pd
import numpy as np
...
LAG_LIST = [1]  # 只用lag1
best_iters = {'total_purchase_amt': 2 if DEBUG else 4, 'total_redeem_amt': 2 if DEBUG else 4}  # 调试时训练轮数极小

# 目标变量变换
APPLY_LOG1P = True  # 是否对目标做log1p变换

# ========== 路径与常量 ==========
FEATURE_PATH = '8.0.4_feature_output/8.0.4_features.parquet'
OUTPUT_DIR = '8.0.4_lgbm_output'
PREDICT_CSV = f'{OUTPUT_DIR}/8.0.4_lgbm_predict.csv'
PLOT_PATH = f'{OUTPUT_DIR}/8.0.4_lgbm_compare.png'
...
if os.path.exists(FEATURE_PATH):
    ...
    os.makedirs('8.0.4_feature_output', exist_ok=True)
...
# 恢复特征耦合：两个模型都使用所有lag特征
all_features = [c for c in train_df.columns if c not in [USER_ID, DATE, 'month']]
# 剔除滑动窗口特征(mean, sum, std)，因为它们的递归更新逻辑复杂，暂时不使用
features_no_rolling = [c for c in all_features if not ('_mean_' in c or '_sum_' in c or '_std_' in c)]
# 最终使用的特征集，包含目标列，在训练时需分别剔除
final_features_pool = [c for c in features_no_rolling if c not in [TARGET_PURCHASE, TARGET_REDEEM]]

purchase_features = final_features_pool
...
redeem_features = final_features_pool
...
# ========== 统一耦合递归预测主流程 ==========
def unified_recursive_predict(p_model, r_model, base_df, p_features, r_features, predict_dates, user_col, date_col):
    """
    :function: (统一耦合版)递归预测主流程，正确处理模型间依赖
    :param p_model: 训练好的申购模型
    :param r_model: 训练好的赎回模型
    :param base_df: 包含历史真实数据的DataFrame
    :param p_features: 申购模型特征列表
    :param r_features: 赎回模型特征列表
    :param predict_dates: 预测日期列表
    :return: 申购预测结果DataFrame, 赎回预测结果DataFrame
    """
    df = base_df.copy()
    p_results, r_results = [], []
    ...
    for idx, d in enumerate(tqdm(predict_dates, desc='统一耦合递归预测')):
        day_idx_mask = df[date_col] == d
        day_df = df.loc[day_idx_mask]
        if day_df.empty:
            continue

        # 1. 预测当天的申购和赎回
        p_preds = inverse_transform_target(p_model.predict(day_df[p_features])).clip(0)
        r_preds = inverse_transform_target(r_model.predict(day_df[r_features])).clip(0)

        # 2. 保存当天结果
        p_results.append(pd.DataFrame({user_col: day_df[user_col], date_col: d, 'pred': p_preds}))
        r_results.append(pd.DataFrame({user_col: day_df[user_col], date_col: d, 'pred': r_preds}))

        # 3. 将当天预测结果更新到主DataFrame中，用于未来日期的特征计算
        df.loc[day_idx_mask, TARGET_PURCHASE] = p_preds
        df.loc[day_idx_mask, TARGET_REDEEM] = r_preds

        # 4. 立即更新当天的 lag1
        df.loc[day_idx_mask, f'{TARGET_PURCHASE}_lag1'] = p_preds
        df.loc[day_idx_mask, f'{TARGET_REDEEM}_lag1'] = r_preds

    return pd.concat(p_results, ignore_index=True), pd.concat(r_results, ignore_index=True)

# ========== 递归预测申购/赎回 ==========
print('开始统一耦合递归预测...')
purchase_pred, redeem_pred = unified_recursive_predict(
    purchase_model, redeem_model, df,
    purchase_features, redeem_features,
    predict_dates, USER_ID, DATE
)
...
def save_predict_csv(p_pred, r_pred, path):
    """
    :function: 聚合每日总量并输出竞赛格式的预测csv
    :param p_pred: 申购预测DataFrame（user_id, ds, pred）
    :param r_pred: 赎回预测DataFrame（user_id, ds, pred）
    :param path: 输出文件路径
    """
...
def plot_compare(purchase_pred, redeem_pred, df_all):
    ...
    # 聚合预测值（修正：用report_date分组）
    purchase_daily = purchase_pred.groupby('report_date')['pred'].sum().reset_index()
    redeem_daily = redeem_pred.groupby('report_date')['pred'].sum().reset_index()
    # 合并
...
if not DEBUG:
    # 修正：确保所有日期字段格式一致
    purchase_pred['ds'] = pd.to_datetime(purchase_pred['ds'])
    redeem_pred['ds'] = pd.to_datetime(redeem_pred['ds'])
    purchase_pred['report_date'] = purchase_pred['ds'].dt.strftime('%Y%m%d')
    redeem_pred['report_date'] = redeem_pred['ds'].dt.strftime('%Y%m%d')
    plot_compare(purchase_pred, redeem_pred, df)

# ========== 说明 ==========
# 1. 本脚本为统一耦合递归版，解决了模型间依赖的核心问题。
# 2. 后续可尝试重新引入滑动窗口特征，但这需要实现更复杂的递归更新逻辑。
# 3. 本脚本包含完整的、独立的特征工程，不依赖旧版本。

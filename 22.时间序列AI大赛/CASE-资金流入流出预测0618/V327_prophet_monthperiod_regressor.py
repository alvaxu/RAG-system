'''
程序说明：
## 1. 本程序在3.2.5_prophet_model(122).py基础上，直接从原始数据出发，增加month_period（月内分段特征）作为回归特征（regressor）参与Prophet建模。
## 2. month_period特征已one-hot编码，并在建模、预测时作为外生变量参与。
## 3. 所有输出文件和目录升级为3.2.7前缀，原有分析、可视化等功能全部保留。
'''
...
# 创建输出目录
os.makedirs('3.2.7_prophet_output', exist_ok=True)
os.makedirs('3.2.7_feature_output', exist_ok=True)
...
holiday_df.to_csv('3.2.7_feature_output/3.2.7_prophet_holidays.csv', index=False, encoding='utf-8-sig')
...
# ========== month_period one-hot编码 ==========
daily = pd.get_dummies(daily, columns=['month_period'])
# 保存带有one-hot特征的特征文件
daily.to_csv('3.2.7_feature_output/3.2.7_prophet_daily_features.csv', index=False, encoding='utf-8-sig')
...
# 申购数据
purchase_df = daily[['ds', 'purchase', 'month_period_begin', 'month_period_middle', 'month_period_end']].copy()
redeem_df = daily[['ds', 'redeem', 'month_period_begin', 'month_period_middle', 'month_period_end']].copy()

holidays = pd.read_csv('3.2.7_feature_output/3.2.7_prophet_holidays.csv', parse_dates=['ds'])
...
# 构造全量特征表用于预测
predict_df = daily[(daily['ds'] >= train_start) & (daily['ds'] <= predict_end)][['ds', 'month_period_begin', 'month_period_middle', 'month_period_end']].reset_index(drop=True)
...
purchase_model.fit(purchase_train.rename(columns={'purchase':'y'}))
...
plt.plot(redeem_df[(redeem_df['ds'] >= train_start) & (redeem_df['ds'] <= train_end)]['ds'],
         redeem_df[(redeem_df['ds'] >= train_start) & (redeem_df['ds'] <= train_end)]['redeem'],
...
plt.savefig('3.2.7_prophet_output/3.2.7_prophet_purchase_redeem_compare.png')
plt.close()

# 7. 分解图
fig = purchase_model.plot_components(purchase_forecast)
plt.savefig('3.2.7_prophet_output/3.2.7_prophet_purchase_components.png')
plt.close()
fig = redeem_model.plot_components(redeem_forecast)
plt.savefig('3.2.7_prophet_output/3.2.7_prophet_redeem_components.png')
plt.close()

# 8. 残差分析
# 申购残差
real_purchase = purchase_df[(purchase_df['ds'] >= train_start) & (purchase_df['ds'] <= train_end)]['purchase'].values
...
plt.savefig('3.2.7_prophet_output/3.2.7_prophet_purchase_residuals.png')
plt.close()
# 赎回残差
real_redeem = redeem_df[(redeem_df['ds'] >= train_start) & (redeem_df['ds'] <= train_end)]['redeem'].values
...
plt.axhline(0, color='red', linestyle='--')
plt.savefig('3.2.7_prophet_output/3.2.7_prophet_redeem_residuals.png')
plt.close()
...
predict_result.to_csv('3.2.7_prophet_output/3.2.7_prophet_predict.csv', index=False, header=False, encoding='utf-8-sig')

print('Prophet建模（含month_period回归特征）、分解图与残差分析已完成，结果保存在3.2.7_prophet_output目录。') 
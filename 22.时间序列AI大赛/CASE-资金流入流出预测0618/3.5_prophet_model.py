...
# 创建输出目录
os.makedirs('3.5_prophet_output', exist_ok=True)
os.makedirs('3.5_feature_output', exist_ok=True)
...
def main():
    ...
    purchase_df.to_csv('3.5_feature_output/3.5_prophet_purchase.csv', index=False, encoding='utf-8-sig')
    redeem_df.to_csv('3.5_feature_output/3.5_prophet_redeem.csv', index=False, encoding='utf-8-sig')
    ...
    holiday_df.to_csv('3.5_feature_output/3.5_prophet_holidays.csv', index=False, encoding='utf-8-sig')
    ...
    daily.to_csv('3.5_feature_output/3.5_prophet_daily_features.csv', index=False, encoding='utf-8-sig')

    # 6. Prophet自动调参与建模函数（与3.2一致）
    def auto_prophet(train_df, holidays, all_dates, label, out_prefix):
        ...
        plt.savefig(f'3.5_prophet_output/3.5_prophet_{out_prefix}_components.png')
        plt.close()
    ...
    purchase_df = pd.read_csv('3.5_feature_output/3.5_prophet_purchase.csv', parse_dates=['ds'])
    redeem_df = pd.read_csv('3.5_feature_output/3.5_prophet_redeem.csv', parse_dates=['ds'])
    holidays = pd.read_csv('3.5_feature_output/3.5_prophet_holidays.csv', parse_dates=['ds'])
    ...
    plt.tight_layout()
    plt.savefig('3.5_prophet_output/3.5_prophet_purchase_redeem_compare.png')
    plt.close()
    ...
    plt.savefig('3.5_prophet_output/3.5_prophet_purchase_residuals.png')
    plt.close()
    # 赎回残差
    ...
    plt.axhline(0, color='red', linestyle='--')
    plt.savefig('3.5_prophet_output/3.5_prophet_redeem_residuals.png')
    plt.close()
    ...
    predict_df.to_csv('3.5_prophet_output/3.5_prophet_predict.csv', index=False, header=False, encoding='utf-8-sig')

    # 13. 保存最优参数和RMSE到txt文件
    with open('3.5_prophet_output/3.5_prophet_best_params.txt', 'w', encoding='utf-8') as f:
    ...
    print('Prophet自动调参建模、分解图与残差分析已完成，结果保存在3.5_prophet_output目录。')

'''
5.1_cycle_prophet_groupmean_fusion.py
功能：
- Prophet模型与周期因子分组均值法加权融合预测。与3.2.1和4.0完全一致
- 自动调优融合权重alpha（在8月验证集上），输出最优alpha、融合预测结果、对比图、RMSE等。
最优融合权重alpha=0.65, 8月验证集RMSE=97354951.16
已输出融合预测结果csv：5.1_feature_output/5.1_cycle_prophet_groupmean_fusion_predict.csv
已输出对比图：5.1_feature_output/5.1_cycle_prophet_groupmean_fusion_compare.png
...
# ========== 1. 数据准备 ==========
os.makedirs('5.1_feature_output', exist_ok=True)
...
best_rmse = float('inf')
for a in alpha_grid:
    val_pred_purchase = a * purchase_prophet_pred[val_idx] + (1-a) * purchase_groupmean_pred[val_idx]
    val_pred_redeem = a * redeem_prophet_pred[val_idx] + (1-a) * redeem_groupmean_pred[val_idx]
    ...
    if rmse < best_rmse:
        best_rmse = rmse
        best_alpha = a
print(f'最优融合权重alpha={best_alpha:.2f}, 8月验证集RMSE={best_rmse:.2f}')

# 用最优alpha做全区间融合
purchase_fusion = best_alpha * purchase_prophet_pred + (1-best_alpha) * purchase_groupmean_pred
redeem_fusion = best_alpha * redeem_prophet_pred + (1-best_alpha) * redeem_groupmean_pred
...
predict_df_out.to_csv('5.1_feature_output/5.1_cycle_prophet_groupmean_fusion_predict.csv', index=False, header=False, encoding='utf-8-sig')
print('已输出融合预测结果csv：5.1_feature_output/5.1_cycle_prophet_groupmean_fusion_predict.csv')
...
plt.tight_layout()
plt.savefig('5.1_feature_output/5.1_cycle_prophet_groupmean_fusion_compare.png')
plt.close()
print('已输出对比图：5.1_feature_output/5.1_cycle_prophet_groupmean_fusion_compare.png')

'''
4.3_cycle_groupmean_predict.py
功能：
- 一键完成周期性特征工程与基于5周期因子分组均值的申购/赎回预测。
- 固定采用 ['weekday', 'is_workday_shift', 'month_period', 'is_holiday', 'is_next_workday'] 这5个周期因子，按顺序多级降级。
...
- 输出2014-03-01~2014-09-30区间真实值与预测值对比图（上下子图），图片保存到4.3_feature_output/4.3_cycle_groupmean_purchase_redeem_compare.png。
- 结果输出到4.3_cycle_groupmean_predict.csv，便于与Prophet等模型对比。
分组特征: ['weekday', 'is_workday_shift', 'month_period', 'is_holiday', 'is_next_workday']，多级降级：依次去掉最后一个特征，最后用全局均值。
成绩：118.1516
'''
...
# ========== 1. 周期性特征工程 ==========
# 创建输出目录
os.makedirs('4.3_feature_output', exist_ok=True)
...
holiday_df.to_csv('4.3_feature_output/4.3_cycle_holidays.csv', index=False, encoding='utf-8-sig')
...
feature_df.to_csv('4.3_feature_output/4.3_cycle_daily_features.csv', index=False, encoding='utf-8-sig')

print('周期性特征工程已完成，特征文件和节假日文件已输出到4.3_feature_output目录。')

# ========== 2. 基于周期因子分组均值的预测 ==========

# 读取刚生成的特征文件
feature_df = pd.read_csv('4.3_feature_output/4.3_cycle_daily_features.csv', parse_dates=['ds'])
...
# ========== 多级降级分组均值预测 ==========
group_cols_full = ['weekday', 'is_workday_shift', 'month_period', 'is_holiday', 'is_next_workday']
group_means = {}
for i in range(len(group_cols_full), 0, -1):
    group = group_cols_full[:i]
    group_means[tuple(group)] = train_df.groupby(list(group))[['purchase','redeem']].mean().reset_index()

# 预测函数，依次降级查找
def predict_row(row):
    for i in range(len(group_cols_full), 0, -1):
        group = group_cols_full[:i]
        df_group = group_means[tuple(group)]
        cond = np.ones(len(df_group), dtype=bool)
        for col in group:
            cond = cond & (df_group[col] == row[col])
        match = df_group[cond]
        if not match.empty:
...
output.to_csv('4.3_feature_output/4.3_cycle_groupmean_predict.csv', index=False, header=False, encoding='utf-8-sig')

print('基于5周期因子的多级降级分组均值预测已完成，结果已输出到4.3_feature_output/4.3_cycle_groupmean_predict.csv。')
...
axes[0].set_title('申购：真实值、in-sample预测与9月预测（5因子多级降级分组均值法）')
axes[0].legend()
# 赎回
...
axes[1].set_title('赎回：真实值、in-sample预测与9月预测（5因子多级降级分组均值法）')
axes[1].legend()
plt.xlim([plot_start, plot_end])
plt.tight_layout()
plt.savefig('4.3_feature_output/4.3_cycle_groupmean_purchase_redeem_compare.png')
plt.close()
print('已输出真实值与预测值对比图：4.3_feature_output/4.3_cycle_groupmean_purchase_redeem_compare.png') 
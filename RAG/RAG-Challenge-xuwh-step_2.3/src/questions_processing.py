...
class QuestionsProcessor:
    def __init__(
        ...
        force_company_name: Optional[str] = None  # 新增参数：强制公司名
    ):
        ...
        self.force_company_name = force_company_name
    ...
    def _format_retrieval_results(self, retrieval_results) -> str:
        ...
        for result in retrieval_results:
            ...
            text = result['text']
            context_parts.append(f'Text retrieved from file {file_name}, page {page_number}:\n"""\n{text}\n"""')
        return "\n\n---\n\n".join(context_parts)
    ...
    def get_answer_for_company(self, company_name: str, question: str, schema: str) -> dict:
        ...
        # 新增：解析LLM返回的final_answer字段中的markdown代码块JSON字符串
        if isinstance(answer_dict.get('final_answer'), str):
            s = answer_dict['final_answer']
            match = re.search(r"```(?:json)?\s*([\s\S]+?)\s*```", s)
            if match:
                try:
                    parsed = json.loads(match.group(1))
                    if isinstance(parsed, dict) and 'step_by_step_analysis' in parsed:
                        answer_dict = parsed
                except Exception:
                    pass
        if self.new_challenge_pipeline:
            # 新版：relevant_pages为[{file_name, page}]
            pages = answer_dict.get("relevant_pages", [])
            # 校验：只保留检索结果中真实存在的file_name+page
            valid_pairs = set((r.get("file_name"), r.get("page")) for r in retrieval_results)
            validated_pages = [p for p in pages if (p.get("file_name"), p.get("page")) in valid_pairs]
            answer_dict["relevant_pages"] = validated_pages
            # references同步
            answer_dict["references"] = [
                {"file_name": p.get("file_name"), "page_index": p.get("page", p.get("page_index"))}
                for p in validated_pages
            ]
        # 新增：插入relevant_file字段
        # 检索到的文档file_name
        relevant_file = None
        if self.llm_reranking:
            # HybridRetriever封装了VectorRetriever，直接用vector_retriever
            retriever_obj = retriever.vector_retriever
        else:
            retriever_obj = retriever
        # all_dbs中找到company_name对应的文档
        for report in retriever_obj.all_dbs:
            document = report.get("document", {})
            metainfo = document.get("metainfo", {})
            if metainfo.get("company_name") == company_name or company_name in metainfo.get("file_name", ""):
                relevant_file = metainfo.get("file_name")
                break
        if relevant_file:
            answer_dict["relevant_file"] = relevant_file
        print("answer_dict:", json.dumps(answer_dict, ensure_ascii=False, indent=2))
    ...
    def process_question(self, question: str, schema: str):
        # 处理单个问题，支持多公司比较
        if self.force_company_name:
            extracted_companies = [self.force_company_name]
    ...
    def _process_single_question(self, question_data: dict) -> dict:
        ...
        try:
            ...
            # 保证relevant_file字段传递到processed_questions
            if "relevant_file" in answer_dict:
                question_data["relevant_file"] = answer_dict["relevant_file"]
            if "error" in answer_dict:
                ...
                if self.new_challenge_pipeline:
                    return {
                        ...
                        **({"relevant_file": answer_dict["relevant_file"]} if "relevant_file" in answer_dict else {})
                    }
                else:
                    return {
                ...
                # 正常返回时也带上relevant_file
                if self.new_challenge_pipeline:
                    return {
                        "question_text": question_text,
                        "kind": schema,
                        "value": answer_dict.get("final_answer") or answer_dict.get("value"),
                        "references": answer_dict.get("references", []),
                        "answer_details": {"$ref": self._create_answer_detail_ref(answer_dict, question_index)},
                        **({"relevant_file": answer_dict["relevant_file"]} if "relevant_file" in answer_dict else {})
                    }
                else:
                    return {
                        "question": question_text,
                        "schema": schema,
                        "answer": answer_dict.get("final_answer") or answer_dict.get("value"),
                        "references": answer_dict.get("references", []),
                        "answer_details": {"$ref": self._create_answer_detail_ref(answer_dict, question_index)},
                        **({"relevant_file": answer_dict["relevant_file"]} if "relevant_file" in answer_dict else {})
                    }
    ...
    def _post_process_submission_answers(self, processed_questions: List[dict]) -> List[dict]:
        ...
        for q in processed_questions:
                ...
                references = [
                    {
                        ...
                        "page_index": ref["page_index"]  # 保持1-based
                    }
            ...
            if "relevant_file" in q:
                submission_answer["relevant_file"] = q["relevant_file"]
            if "relevant_pages" in q:
                submission_answer["relevant_pages"] = q["relevant_pages"]
            submission_answer["references"] = references
    ...
    def process_single_question(self, question: str, kind: str = "string"):
        ...
        if len(extracted_companies) == 1:
            ...
            answer_dict['company_name'] = company_name  # 新增：确保answer带有company_name
            return answer_dict
        else:
            print("[计时] [单问] 开始多公司比较 ...")
            t2 = time.time()
...
class BaseDashscopeProcessor:
    ...
    def send_message(
        ...
        **kwargs
        ...
        # print('dashscope.api_key=', dashscope.api_key)  # 删除此行
        print('model=', model)

...
class Pipeline:
    ...
    def chunk_reports(self, include_serialized_tables: bool = False):
        ...
        """
        text_splitter = TextSplitter(splitter_model_name=self.run_config.answering_model)
    ...
    def create_vector_dbs(self):
        """从合并后的分块报告创建向量数据库，只处理chunked_reports_merged目录下的json文件"""
        # 新增：只处理合并后的大json
        input_dir = self.paths.documents_dir.parent / "chunked_reports_merged"
        output_dir = self.paths.vector_db_dir
        vdb_ingestor = VectorDBIngestor()
        vdb_ingestor.process_reports(input_dir, output_dir)
        print(f"Vector databases created in {output_dir} (from merged chunks in {input_dir})")
    ...
    def process_questions(self):
        processor = QuestionsProcessor(
            ...
            force_company_name="AI应用开发"  # 强制公司名
        )
    ...
    def merge_chunks_by_company(self):
        """
        合并同一公司（sha1）下的所有分块json为一个大json，便于与公司级faiss索引一一对应。
        合并后的大json输出到 databases/chunked_reports_merged 目录。
        输出文件名为 {sha1}_{company_name}.json，内容为所有分块的合并list，保留原file_name、page等信息。
        metainfo中file_name改为file_names（所有文档名列表），统计量合并。
        """
        import json
        from collections import defaultdict
        import re
        input_dir = self.paths.documents_dir
        # 新增：合并输出目录
        merged_dir = input_dir.parent / "chunked_reports_merged"
        merged_dir.mkdir(parents=True, exist_ok=True)
        # 1. 按sha1分组收集所有分块和元信息
        sha1_to_chunks = defaultdict(list)
        sha1_to_file_names = defaultdict(list)
        sha1_to_stats = defaultdict(lambda: {
            "pages_amount": 0,
        ...
        for json_path in input_dir.glob("*.json"):
            ...
            file_name = metainfo.get("file_name", None)
            sha1 = metainfo.get("sha1", None)
            company_name = metainfo.get("company_name", None)
            if not sha1 or not file_name:
                continue
            sha1_to_company[sha1] = company_name
            # 统计所有文档名，去重
            if file_name not in sha1_to_file_names[sha1]:
                sha1_to_file_names[sha1].append(file_name)
            # 合并统计量
            stats = sha1_to_stats[sha1]
            stats["pages_amount"] += metainfo.get("pages_amount", 0)
            stats["text_blocks_amount"] += metainfo.get("text_blocks_amount", 0)
            stats["tables_amount"] += metainfo.get("tables_amount", 0)
            stats["pictures_amount"] += metainfo.get("pictures_amount", 0)
            stats["equations_amount"] += metainfo.get("equations_amount", 0)
            stats["footnotes_amount"] += metainfo.get("footnotes_amount", 0)
            # 合并分块，补充file_name字段
            chunks = data.get("content", {}).get("chunks", [])
            for chunk in chunks:
                chunk["file_name"] = file_name
            sha1_to_chunks[sha1].extend(chunks)
        # 2. 合并并输出
        for sha1, chunks in sha1_to_chunks.items():
            company_name = sha1_to_company.get(sha1, "")
            file_names = list(dict.fromkeys(sha1_to_file_names[sha1]))  # 去重保序
            stats = sha1_to_stats[sha1]
            metainfo = {
                "sha1": sha1,
                "company_name": company_name,
                "file_names": file_names,
                "pages_amount": stats["pages_amount"],
                "text_blocks_amount": stats["text_blocks_amount"],
                "tables_amount": stats["tables_amount"],
                "pictures_amount": stats["pictures_amount"],
                "equations_amount": stats["equations_amount"],
                "footnotes_amount": stats["footnotes_amount"]
            }
            merged = {
                "metainfo": metainfo,
                "content": {"chunks": chunks}
            }
            # 输出文件名：sha1_公司名.json，去除公司名中的特殊字符
            safe_company = re.sub(r'[^\w\u4e00-\u9fa5]+', '', company_name) if company_name else 'unknown'
            out_path = merged_dir / f"{sha1}_{safe_company}.json"
            with open(out_path, 'w', encoding='utf-8') as f:
                json.dump(merged, f, ensure_ascii=False, indent=2)
            print(f"已合并输出: {out_path.resolve()}，共{len(chunks)}个分块，涉及文档: {file_names}")
...
if __name__ == "__main__":
    ...
    # root_path = here() / "RAG-Challenge-xuwh-step_2.2" / "data" / "test_set"
    ...
    # minerU_api_key = 'eyJ0eXBlIjoiSldUIiwiYWxnIjoiSFM1MTIifQ..UI_zyhxUtruBm5UUKoZqpccyu5EmS05yw9OOb4zOM-xKo11FkurTqVqFfP-1E2LZ3ue7aLzjMWEodWq66wOxYQ'

    # pipeline.export_reports_to_markdown_mineru(api_key=minerU_api_key, language='ch')
    
    # # 5. 将md报告分块，便于后续向量化，输出到 databases/chunked_reports
    # print('5. 将md报告分块，便于后续向量化，输出到 databases/chunked_reports')
    # pipeline.chunk_reports() 
    # 新增：合并分块json,输出到 databases/chunked_reports_merged 
    print('5.5 合并同公司分块json，生成公司级大json')
    pipeline.merge_chunks_by_company()

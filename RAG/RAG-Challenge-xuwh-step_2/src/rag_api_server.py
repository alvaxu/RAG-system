...
def get_answer(question):
    ...
    # 如果 answer['final_answer'] 是字符串化的 JSON，要先解析出来
    if isinstance(answer.get('final_answer'), str):
        try:
            # 尝试解析字符串为 dict
            parsed = json.loads(answer['final_answer'])
            # 如果解析出来是 dict，并且有 step_by_step_analysis 等字段，直接用它替换 answer

...
| ------ | -------- | -------- | -------- | -------- |
| `extend_config()` | 扩展配置，添加RAG节点 | 无 | 扩展后的配置字典 | ConfigManager |
| `get_rag_config(key, default)` | 获取RAG配置值 | 配置键、默认值 | 配置值或默认值 | ConfigManager |
| `validate_rag_config()` | 验证RAG配置完整性 | 无 | 验证结果字典 | ConfigManager |
| `reload_rag_config()` | 重新加载RAG配置 | 无 | 重载结果布尔值 | ConfigManager |
| `get_rag_engine_config(engine_name)` | 获取特定引擎配置 | 引擎名称 | 引擎配置字典 | ConfigManager |
| `get_rag_model_config(model_type)` | 获取模型配置 | 模型类型 | 模型配置字典 | ConfigManager |
| `export_rag_config(export_path)` | 导出RAG配置 | 导出路径 | 导出结果布尔值 | ConfigManager |
| `import_rag_config(import_path)` | 导入RAG配置 | 导入路径 | 导入结果布尔值 | ConfigManager |
...
class ConfigManager:
    ...
    def extend_config(self) -> bool:
        ...
        try:
            ...
            logging.info("配置扩展成功，已添加RAG系统配置节点")
            return True
            
        except Exception as e:
            logging.error(f"配置扩展失败: {e}")
            return False

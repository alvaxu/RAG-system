...
10. **关联建立**：建立分块与原始大表的关联关系
11. **重组支持**：支持RAG查询时重组完整大表
12. **完整性验证**：验证分块完整性和连续性
...
**大表分块和重组功能**：
```python
def _process_table_chunking(self, content_result: Dict) -> List[Dict]:
    """
    智能表格分块处理
    
    功能：
    1. 根据配置的chunk_size自动分块
    2. 保持表头完整性
    3. 确保行数连续性
    4. 生成分块关联信息
    """
    try:
        table_body = content_result.get('table_body', '')
        ...
        # 智能分块处理
        chunks = self._smart_table_chunking_html(table_body, chunk_size)
        
        # 为每个分块生成元数据
        chunked_results = []
        for i, chunk_html in enumerate(chunks):
            chunk_result = content_result.copy()
            chunk_result.update({
                'table_body': chunk_html,
                'is_subtable': True,
                'parent_table_id': f"table_{int(time.time())}",
                'subtable_index': i,
                'chunk_start_row': i * self._estimate_rows_per_chunk(chunk_size),
                'chunk_end_row': (i + 1) * self._estimate_rows_per_chunk(chunk_size)
            })
        ...
        logging.error(f"表格分块处理失败: {e}")
        return [content_result]

def _smart_table_chunking_html(self, table_html: str, chunk_size: int) -> List[str]:
    """
    HTML表格智能分块
...
CHUNK_METADATA_FIELDS = {
    # 分块标识
    'is_subtable': True,                    # 标记为子表
    'parent_table_id': 'str',               # 父表ID
    'subtable_index': 'int',                # 子表索引（0, 1, 2, ...）
    
    # 行数范围
    'chunk_start_row': 'int',               # 分块起始行
    'chunk_end_row': 'int',                 # 分块结束行
    
    # 分块内容
    'table_body': 'str',                    # 分块HTML内容
    'table_content': 'str',                 # 分块纯文本内容
}
```
...
**重组后的表格特点**：
```python
RECONSTRUCTED_TABLE_FEATURES = {
    # 标识字段
    'reconstructed': True,                  # 标记为重组后的表格
    'original_subtables': ['list'],         # 原始子表ID列表
    'reconstruction_timestamp': 'int',      # 重组时间戳
    
    # 内容字段
    'table_body': 'str',                    # 完整的HTML表格
    'table_content': 'str',                 # 完整的纯文本内容
    'table_rows': 'int',                    # 总行数
    
    # 状态字段
    'is_subtable': False,                   # 不再是子表
    'parent_table_id': None,                # 没有父表
...
```python
# 1. 分块查询（精确匹配）
def search_table_chunks(self, query: str, top_k: int = 5) -> List[Dict]:
    """查询表格分块，适合精确信息检索"""
    # 在分块中进行向量相似度搜索
    pass

# 2. 完整表格查询（全局分析）
def search_complete_tables(self, query: str, top_k: int = 5) -> List[Dict]:
    """查询完整表格，适合全局分析和理解"""
    # 先查询分块，再重组完整表格
    pass

# 3. 混合查询策略
def search_tables_hybrid(self, query: str, strategy: str = 'auto') -> List[Dict]:
    """
    混合查询策略
    
    策略选项：
    - 'chunks_only': 只查询分块
    - 'complete_only': 只查询完整表格
    - 'auto': 根据查询类型自动选择
    - 'both': 同时返回分块和完整表格
    """
    pass
...
# 用户查询："2023年Q1的营收数据"
# 策略：分块查询，快速定位到具体数据行
result = search_table_chunks("2023年Q1营收数据")

# 场景2：表格整体分析
# 用户查询："分析这个表格的趋势"
# 策略：完整表格查询，重组后进行分析
result = search_complete_tables("分析表格趋势")

# 场景3：混合查询
# 用户查询："这个表格的总体情况和具体数据"
# 策略：同时返回分块和完整表格
result = search_tables_hybrid("表格总体情况和具体数据", "both")
```
...
class TableProcessor:
    ...
    def _combine_table_text(self, content_result: Dict) -> str:
        ...
        # 添加表格内容
        if content_result.get('table_body'):
            parts.append(content_result['table_body'])
    ...
    def _build_final_result(self, structure: Dict, content: Dict, 
        ...
        result = {
            ...
            'table_body': content.get('table_body', ''),       # 表格HTML内容（用于向量化）
            'table_caption': content.get('table_caption', []), # 表格标题
            'table_footnote': content.get('table_footnote', []), # 表格脚注
...
class TableFormatter:
    ...
    def process_chunking(self, content_result: Dict) -> Dict:
        ...
        """
        try:
            table_content = content_result.get('table_body', '')
    ...
    def _generate_single_table_html(self, chunk: Dict) -> str:
        ...
        try:
            ...
            content_lines = chunk.get('table_body', '').split('\n')

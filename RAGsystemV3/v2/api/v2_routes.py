...
def v2_ask_question():
    ...
    try:
        ...
        if hasattr(result, 'results') and result.results:
            ...
            logger.info(f"🔍 开始处理 {len(result.results)} 个结果，构建image_results和table_results")
            
            for i, doc in enumerate(result.results):
                try:
                    logger.info(f"🔍 处理结果 {i+1}: {type(doc)}")
                    logger.info(f"🔍 结果 {i+1} 的键: {list(doc.keys()) if isinstance(doc, dict) else 'N/A'}")
                    ...
                    logger.info(f"🔍 结果 {i+1} - chunk_type: '{chunk_type}'")
                    
                    if chunk_type == 'image':
                        logger.info(f"🔍 结果 {i+1} - 识别为图片类型")
                        # 使用统一的图片字段映射
                        image_result = _build_unified_image_result(actual_doc, score)
                        image_results.append(image_result)
                        
                    elif chunk_type == 'table':
                        logger.info(f"🔍 结果 {i+1} - 识别为表格类型")
                        # 使用统一的表格字段映射
                        ...
                        logger.warning(f"🔍 结果 {i+1} - 未知的chunk_type: '{chunk_type}'")
...
def _extract_actual_doc_and_score(doc):
    """
    提取实际的Document对象和分数 - 增强版本，支持字典格式和Document对象
    
    :param doc: 文档对象（可能是Document对象或字典）
    ...
    try:
        ...
        # 情况2：向后兼容：处理简单嵌套 {'doc': Document(...)}
        elif isinstance(doc, dict) and 'doc' in doc:
            return _extract_actual_doc_and_score(doc['doc'])
        
        # 情况3：表格引擎返回的字典格式（新增）
        elif isinstance(doc, dict) and 'chunk_type' in doc:
            # 创建MockDocument对象来兼容现有处理逻辑
            class MockDocument:
                def __init__(self, doc_dict):
                    self.metadata = {
                        'document_name': doc_dict.get('document_name', '未知文档'),
                        'page_number': doc_dict.get('page_number', '未知页'),
                        'chunk_type': doc_dict.get('chunk_type', '未知类型'),
                        'table_type': doc_dict.get('table_type', 'unknown'),
                        'doc_id': doc_dict.get('doc_id', doc_dict.get('id', 'unknown')),
                        'table_id': doc_dict.get('table_id', doc_dict.get('doc_id', doc_dict.get('id', 'unknown'))),  # 🔑 修复：添加table_id字段
                        'page_content': doc_dict.get('table_html', ''),  # 🔑 修复：table_html -> page_content
                        'processed_table_content': doc_dict.get('table_content', ''),  # 🔑 修复：table_content -> processed_table_content
                        'table_headers': doc_dict.get('table_headers', []),
                        'table_row_count': doc_dict.get('table_row_count', 0),
                        'table_column_count': doc_dict.get('table_column_count', 0),
                        'table_summary': doc_dict.get('table_summary', ''),
                        'chunk_index': doc_dict.get('chunk_index', 0)
                    }
                    self.page_content = doc_dict.get('table_html', '')  # 🔑 修复：table_html -> page_content
                    self.score = doc_dict.get('score', 0.0)
            
            mock_doc = MockDocument(doc)
            return mock_doc, doc.get('score', 0.0)

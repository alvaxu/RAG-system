...
class TextEngine(BaseEngine):
    ...
    def _process_text_result(self, doc, query: str) -> Dict[str, Any]:
        ...
        return {
            # 文本字段 - 明确对应
            'content': content,                                              # 明确从page_content获取
            'content_preview': content[:200] + '...' if len(content) > 200 else content, # 明确从page_content获取
            'chunk_index': metadata.get('chunk_index', 0),                  # 明确从chunk_index获取
            
            # 通用字段 - 明确对应
            'document_name': metadata.get('document_name', '未知文档'),      # 明确从document_name获取
            'page_number': metadata.get('page_number', '未知页'),            # 明确从page_number获取
            'chunk_type': metadata.get('chunk_type', 'text'),               # 明确从chunk_type获取

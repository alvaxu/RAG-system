# 7.0.1_fusion_model.py
# 功能: 融合Prophet模型(3.2.5)和分组均值模型(4.0)的预测结果
# 1. 统一使用 chinese_calendar 生成节假日特征
# 2. 分别用两种模型生成预测
# 3. 移除自动寻找最优权重的功能，并直接使用您指定的固定权重：申购权重 p_w = 0.5，赎回权重 r_w = 0.5
# 4. 使用权重进行加权融合，生成最终预测结果
# 5. 输出融合结果、最优权重和对比图
...
def setup_environment():
    ...
    os.makedirs('7.0.1_fusion_output', exist_ok=True)
    os.makedirs('7.0.1_feature_output', exist_ok=True)
...
def create_holidays_and_features(start_date='2014-03-01', end_date='2014-09-30'):
    ...
    holidays_df.to_csv('7.0.1_feature_output/7.0.1_holidays.csv', index=False, encoding='utf-8-sig')
    ...
    feature_df.to_csv('7.0.1_feature_output/7.0.1_daily_features.csv', index=False, encoding='utf-8-sig')
...
def main():
    ...
    print("\n--- 固定权重参数 ---")
    print(f"申购权重 (Prophet): {p_w:.2f}, (分组均值): {1-p_w:.2f}")
    print(f"赎回权重 (Prophet): {r_w:.2f}, (分组均值): {1-r_w:.2f}")
    ...
    predict_output[['ds', 'purchase', 'redeem']].to_csv('7.0.1_fusion_output/7.0.1_fusion_predict.csv', index=False, header=False)
    
    # 保存带权重的完整数据
    final_df.to_csv('7.0.1_fusion_output/7.0.1_all_predictions_data.csv', index=False, encoding='utf-8-sig')
    
    # 绘制对比图
    plot_results(final_df, train_end, '7.0.1_fusion_output/7.0.1_fusion_compare.png')

    print("\n融合模型执行完毕！结果已保存在 '7.0.1_fusion_output' 目录中。")

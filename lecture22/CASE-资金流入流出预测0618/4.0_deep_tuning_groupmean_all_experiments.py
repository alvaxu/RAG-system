# 4.0_deep_tuning_groupmean_all_experiments.py
# 功能：分组因子全组合实验（1~5个因子的所有组合），只输出分数日志，自动找出最优分组
# ========== 最优分组因子方案 ==========
# 最优实验ID: 414
...
log_list = []
exp_id = 1
for k in range(1, 6):  # 选1~5个因子
    for group_cols in combinations(all_factors, k):
        ...
        pred_purchase, pred_redeem = zip(*train_df.apply(predict_row, axis=1))
        ...
        if exp_id % 50 == 0:
            print(f'已完成{exp_id}个实验...')
        exp_id += 1

# 输出实验日志
log_df = pd.DataFrame(log_list)
log_df.to_csv('4.0_deep_tuning_output/4.0_deep_tuning_all_experiment_log.csv', index=False, encoding='utf-8-sig')
...
with open('4.0_deep_tuning_output/4.0_deep_tuning_all_best_result.txt', 'w', encoding='utf-8') as f:
    f.write('========== 最优分组因子方案 ==========\n')
    f.write(f"最优实验ID: {best_row['experiment_id']}\n")
    f.write(f"分组因子: {best_row['group_cols']}\n")
    f.write(f"训练区间RMSE: {best_row['total_rmse']:.2f}\n")

print('\n所有实验已完成，实验日志已输出到4.0_deep_tuning_output/4.0_deep_tuning_all_experiment_log.csv') 
'''
4.0_cycle_groupmean_predict.py
功能：
- 一键完成周期性特征工程与基于周期因子分组均值的申购/赎回预测。
- 先生成节假日、调休补班、周期性特征文件（与4.0_cycle_feature_engine.py一致），再进行分组均值预测。
- 主要周期因子：weekday、month_period、is_holiday，调休补班日与普通工作日合并，节后首日单独标记。
- 训练区间2014-03-01~2014-08-31，预测区间2014-09-01~2014-10-10，输出csv区间2014-09-01~2014-09-30。
- 输出2014-03-01~2014-09-30区间真实值与预测值对比图（上下子图），图片保存到4.0_feature_output/4.0_cycle_groupmean_purchase_redeem_compare.png。
- 结果输出到4.0_cycle_groupmean_predict.csv，便于与Prophet等模型对比。
固定采用 ['weekday', 'month_period', 'is_holiday', 'is_next_workday'] 这4个周期因子
先用最细粒度（含is_next_workday=1），找不到则依次降级（去掉month_period，再去掉is_next_workday），最后用全局均值。
分数：119.3120
'''
...
def predict_row(row):
    ...
    # 其次用(weekday, month_period, is_holiday)分组
...
output.to_csv('4.0_feature_output/4.0_cycle_groupmean_predict.csv', index=False, header=False, encoding='utf-8-sig')

print('周期性特征工程与基于周期因子分组均值的预测已完成，结果已输出到4.0_feature_output/4.0_cycle_groupmean_predict.csv。')
...
axes[0].set_title('申购：真实值、in-sample预测与9月预测（分组均值法）')
axes[0].legend()
# 赎回
...
axes[1].set_title('赎回：真实值、in-sample预测与9月预测（分组均值法）')
axes[1].legend()
plt.xlim([plot_start, plot_end])
plt.tight_layout()
plt.savefig('4.0_feature_output/4.0_cycle_groupmean_purchase_redeem_compare.png')
plt.close()
print('已输出真实值与预测值对比图：4.0_feature_output/4.0_cycle_groupmean_purchase_redeem_compare.png') 
'''
3.4.1_prophet_model.py
功能：
- 同3.4_prophet_model.py，区别在于不考虑年周期性，只考虑月周期性。
- 普通周末不会被当作节假日，只有法定节假日和调休补班的周末才会被特殊标记。
- 余额宝申购/赎回数据Prophet建模与预测，直接采用建议参数，不再做参数调优。
...
- 输出预测结果、分解图、残差图。
- 所有输出文件及目录以3.4.1_开头，便于与其他版本对比。
- 适用于直接复现建议参数下Prophet模型的预测效果。
...
# 创建输出目录
os.makedirs('3.4.1_prophet_output', exist_ok=True)
os.makedirs('3.4.1_feature_output', exist_ok=True)
...
purchase_df.to_csv('3.4.1_feature_output/3.4.1_prophet_purchase.csv', index=False, encoding='utf-8-sig')
redeem_df.to_csv('3.4.1_feature_output/3.4.1_prophet_redeem.csv', index=False, encoding='utf-8-sig')
...
holiday_df.to_csv('3.4.1_feature_output/3.4.1_prophet_holidays.csv', index=False, encoding='utf-8-sig')
...
daily.to_csv('3.4.1_feature_output/3.4.1_prophet_daily_features.csv', index=False, encoding='utf-8-sig')
...
purchase_df = pd.read_csv('3.4.1_feature_output/3.4.1_prophet_purchase.csv', parse_dates=['ds'])
redeem_df = pd.read_csv('3.4.1_feature_output/3.4.1_prophet_redeem.csv', parse_dates=['ds'])
holidays = pd.read_csv('3.4.1_feature_output/3.4.1_prophet_holidays.csv', parse_dates=['ds'])
...
plt.savefig('3.4.1_prophet_output/3.4.1_prophet_purchase_redeem_compare.png')
plt.close()

# 10. 分解图
fig = purchase_model.plot_components(purchase_forecast)
plt.savefig('3.4.1_prophet_output/3.4.1_prophet_purchase_components.png')
plt.close()
fig = redeem_model.plot_components(redeem_forecast)
plt.savefig('3.4.1_prophet_output/3.4.1_prophet_redeem_components.png')
plt.close()
...
plt.savefig('3.4.1_prophet_output/3.4.1_prophet_purchase_residuals.png')
plt.close()
# 赎回残差
...
plt.axhline(0, color='red', linestyle='--')
plt.savefig('3.4.1_prophet_output/3.4.1_prophet_redeem_residuals.png')
plt.close()
...
predict_df.to_csv('3.4.1_prophet_output/3.4.1_prophet_predict.csv', index=False, header=False, encoding='utf-8-sig')

print('Prophet建议参数建模、分解图与残差分析已完成，结果保存在3.4.1_prophet_output目录。') 
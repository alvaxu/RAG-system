'''
3.2.1_prophet_model.py
功能：
- 同3.2_prophet_model.py，区别在于，通过设置
yearly_seasonality=False,
m.add_seasonality(name='monthly', period=30.5, fourier_order=5)
不考虑年周期性，只考虑显式定义的月周期性。。
- 普通周末不会被当作节假日，只有法定节假日和调休补班的周末才会被特殊标记。
- 余额宝申购/赎回数据Prophet自动调参建模与预测。
- 采用网格搜索自动调参，对changepoint_prior_scale、seasonality_prior_scale、holidays_prior_scale、seasonality_mode等主参数遍历组合，自动选择验证集RMSE最优参数。
- 对申购和赎回分别进行自动调参、预测、可视化、残差分析和分解图输出。
- 训练区间2014-03-01到2014-08-31，预测区间2014-09-01到2014-09-30。
- 输出最优参数、验证集RMSE、9月预测结果、分解图、残差图。
- 不引入外部变量。

- 最终调优结果可显著提升模型在验证集上的拟合度和9月预测的合理性，但仍受限于Prophet模型本身的趋势外推能力。
- 结果如下：
...
# 创建输出目录
os.makedirs('3.2.1_prophet_output', exist_ok=True)
os.makedirs('3.2.1_feature_output', exist_ok=True)
...
def auto_prophet(train_df, holidays, all_dates, label, out_prefix):
    ...
    plt.savefig(f'3.2.1_prophet_output/3.2.1_prophet_{out_prefix}_components.png')
    plt.close()
...
def main():
    ...
    purchase_df.to_csv('3.2.1_feature_output/3.2.1_prophet_purchase.csv', index=False, encoding='utf-8-sig')
    redeem_df.to_csv('3.2.1_feature_output/3.2.1_prophet_redeem.csv', index=False, encoding='utf-8-sig')
    ...
    holiday_df.to_csv('3.2.1_feature_output/3.2.1_prophet_holidays.csv', index=False, encoding='utf-8-sig')
    ...
    daily.to_csv('3.2.1_feature_output/3.2.1_prophet_daily_features.csv', index=False, encoding='utf-8-sig')
    ...
    purchase_df = pd.read_csv('3.2.1_feature_output/3.2.1_prophet_purchase.csv', parse_dates=['ds'])
    redeem_df = pd.read_csv('3.2.1_feature_output/3.2.1_prophet_redeem.csv', parse_dates=['ds'])
    holidays = pd.read_csv('3.2.1_feature_output/3.2.1_prophet_holidays.csv', parse_dates=['ds'])
    ...
    plt.savefig('3.2.1_prophet_output/3.2.1_prophet_purchase_redeem_compare.png')
    plt.close()
    ...
    plt.savefig('3.2.1_prophet_output/3.2.1_prophet_purchase_residuals.png')
    plt.close()
    # 赎回残差
    ...
    plt.axhline(0, color='red', linestyle='--')
    plt.savefig('3.2.1_prophet_output/3.2.1_prophet_redeem_residuals.png')
    plt.close()
    ...
    predict_df.to_csv('3.2.1_prophet_output/3.2.1_prophet_predict.csv', index=False, header=False, encoding='utf-8-sig')

    # 保存最优参数和RMSE到txt文件
    with open('3.2.1_prophet_output/3.2.1_prophet_best_params.txt', 'w', encoding='utf-8') as f:
    ...
    print('Prophet自动调参建模、分解图与残差分析已完成，结果保存在3.2.1_prophet_output目录。')

...
def main():
    ...
    data['car_age_bin'] = pd.cut(data['car_age'], bins=[-1,1,3,5,8,12,20,50], labels=False)
    data['log_power'] = np.log1p(data['power'])
    data['log_kilometer'] = np.log1p(data['kilometer'])
    data['log_car_age'] = np.log1p(data['car_age'].clip(lower=0))
    ...
    # 4.1 训练集进一步剔除极端高价、极端power样本
    data = data[(data['price'] == -1) | ((data['price'] < 80000) & (data['power'] < 400))]
    ...
    features = [
        ...
        # 新增特征
        'power_bin', 'km_bin', 'car_age_bin', 'log_power', 'log_kilometer', 'log_car_age'
    ]

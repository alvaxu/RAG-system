...
def sarimax_forecast_with_features_and_residuals(balance_csv_path='user_balance_table.csv'):
    ...
    model_purchase_fit = pm.auto_arima(endog_purchase_hist, 
                                       exogenous=exog_hist, 
                                       seasonal=True, 
                                       m=7, 
                                       d=0, # 强制非季节性差分阶数为0
                                       ...
                                       error_action='ignore')
    purchase_order = model_purchase_fit.order
    purchase_seasonal_order = model_purchase_fit.seasonal_order
    print(f"【申购金额】SARIMAX最优参数：非季节性 {purchase_order}, 季节性 {purchase_seasonal_order}")
    ...
    # 手动对赎回金额进行一阶差分
    differenced_redeem_hist = endog_redeem_hist.diff(1).dropna()
    # 相应的外部变量也要与差分后的序列对齐
    exog_hist_redeem_diff = exog_hist.loc[differenced_redeem_hist.index]

    model_redeem_fit = pm.auto_arima(differenced_redeem_hist, # 传入差分后的序列
                                      exogenous=exog_hist_redeem_diff, 
                                      seasonal=True, 
                                      m=7, 
                                      d=0, # 序列已经差分，所以这里设置为0
                                      D=0, # 强制季节性差分阶数为0
                                      ...
                                      suppress_warnings=True, 
                                      error_action='ignore')
    redeem_order = model_redeem_fit.order
    redeem_seasonal_order = model_redeem_fit.seasonal_order
    print(f"【赎回金额】SARIMAX最优参数：非季节性 {redeem_order}, 季节性 {redeem_seasonal_order}")

    model_redeem = SARIMAX(differenced_redeem_hist, # 传入差分后的序列
                           exog=exog_hist_redeem_diff, 
                           order=redeem_order, 
    ...
    predicted_redeem_diff = forecast_redeem_result.predicted_mean # 预测结果是差分后的

    # 对赎回金额预测结果进行逆差分，还原到原始尺度
    # 需要原始序列的最后一个值作为逆差分的起点
    last_redeem_value_hist = endog_redeem_hist.iloc[-1]
    predicted_redeem = np.r_[last_redeem_value_hist, predicted_redeem_diff].cumsum()[1:]
    ...
    plt.tight_layout()
    plt.savefig('4.purchase_residuals_analysis.png') # 添加前缀
    ...
    plt.tight_layout()
    plt.savefig('4.redeem_residuals_analysis.png') # 添加前缀
    # plt.show() # 不在代码中直接显示，由用户决定
    ...
    forecast_df.to_csv('4.sarimax_forecast_with_features_and_residuals.csv', index=False, encoding='utf-8', header=False)
    print('SARIMAX预测结果已保存到 4.sarimax_forecast_with_features_and_residuals.csv')
    ...
    plt.title('【SARIMAX多特征自动调参预测】2014-03~2014-08及未来30天每日申购与赎回金额趋势', fontsize=14)
    plt.legend()
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('4.sarimax_multi_features_trend_201409.png')
    plt.show()

if __name__ == "__main__":
    sarimax_forecast_with_features_and_residuals() 
...
def main():
    ...
    model = CatBoostRegressor(iterations=600, depth=10, learning_rate=0.03, subsample=0.85, random_state=42, verbose=0)
    model.fit(X_train, y_train)
    y_pred_log = model.predict(X_val)
    y_pred = np.expm1(y_pred_log)
    y_val_true = np.expm1(y_val)
    print('CatBoost MAE:', mean_absolute_error(y_val_true, y_pred))
    print('CatBoost RMSE:', np.sqrt(mean_squared_error(y_val_true, y_pred)))

    test_pred_log = model.predict(X_test)
    test_pred = np.expm1(test_pred_log)
    result = test[['SaleID']].copy()
    result['price'] = test_pred
    result.to_csv('used_car_sample_submit_cat.csv', index=False)
    print('CatBoost预测结果已保存到used_car_sample_submit_cat.csv')

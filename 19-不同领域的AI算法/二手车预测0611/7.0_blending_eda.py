...
def main():
    ...
    if drop_unique:
        print(f"剔除唯一值/常量特征: {drop_unique}")
    ...
    print(f"特征选择后剩余特征数: {len(selected_cols)}")
    ...
    # 17. 最终训练特征概览
    print("\n========== 17. 最终训练特征概览 ==========")
    with open("7.0_blending_eda_features.txt", "w", encoding="utf-8") as f:
        f.write("========== 17. 最终训练特征概览 ==========" + "\n")
    ...
    # 18. 划分训练集和验证集（70%模型训练，30%用于blending）
    ...
    print("LightGBM训练完成。")
    joblib.dump(lgb, '7.0_blending_eda_lgb_model.joblib')
    print('LightGBM模型已保存到 7.0_blending_eda_lgb_model.joblib')
    ...
    print("XGBoost训练完成。")
    joblib.dump(xgb, '7.0_blending_eda_xgb_model.joblib')
    print('XGBoost模型已保存到 7.0_blending_eda_xgb_model.joblib')
    ...
    print("CatBoost训练完成。")
    joblib.dump(cat, '7.0_blending_eda_cat_model.joblib')
    print('CatBoost模型已保存到 7.0_blending_eda_cat_model.joblib')
    ...
    result.to_csv('7.0_blending_eda_predict.csv', index=False)
    print('Blending预测结果已保存到 7.0_blending_eda_predict.csv')
    ...
    plt.xlabel('特征重要性')
    plt.title('7.0_blending_eda_XGBoost特征重要性Top50')
    plt.tight_layout()
    plt.savefig('7.0_blending_eda_importance_top50.png', dpi=150)
    plt.close()
    print('特征重要性Top50图已保存为 7.0_blending_eda_importance_top50.png')

"""
11.21_xgb_feature_optimize_v20.py

基于11.20版本，引入v_12与power、v_3与kilometer_log的新交互特征以及car_age与power的比值特征，以期进一步提升模型性能。
"""
import pandas as pd
import numpy as np
import matplotlib
...
def preprocess_and_feature_engineer(train_df, test_df):
    ...
    # 11. v_3 * kilometer_log 交互特征
    if 'v_3' in combined_df.columns and 'kilometer_log' in combined_df.columns:
        combined_df['v_3_kilometer_inter'] = combined_df['v_3'] * combined_df['kilometer_log']
        print("已添加特征 'v_3_kilometer_inter'.")
    else:
        print("警告: 无法添加 'v_3_kilometer_inter'，因为 'v_3' 或 'kilometer_log' 列不存在。")

    # 12. car_age / power 比值特征
    if 'car_age' in combined_df.columns and 'power' in combined_df.columns:
        combined_df['car_age_power_ratio'] = combined_df['car_age'] / combined_df['power'].replace(0, np.nan) # 避免除以零
        combined_df['car_age_power_ratio'].replace([np.inf, -np.inf], np.nan, inplace=True) # 将inf替换为NaN，由后续填充处理
        print("已添加特征 'car_age_power_ratio'.")
    else:
        print("警告: 无法添加 'car_age_power_ratio'，因为 'car_age' 或 'power' 列不存在。")


    # 明确定义最终特征列表 (包括原始数值型特征和所有新增的增强特征)
    # 确保不包含原始的 'brand' 和 'model'，只包含它们的目标编码版本
    # 过滤掉非数值列、is_train、price、SaleID 和 bodyType_missing
    excluded_cols = ['is_train', 'price', 'SaleID', 'bodyType_missing'] 
    ...
    # 确保所有交互特征都在最终特征列表中 (只保留v15和v20新增的)
    ...
    if 'car_age_power_ratio' in combined_df.columns and 'car_age_power_ratio' not in final_features_to_select:
        final_features_to_select.append('car_age_power_ratio')
...
def train_and_evaluate_model(X, y, X_test, test_saleid):
    ...
    plt.xlabel('实际价格')
    plt.ylabel('预测价格')
    plt.savefig('11.21_xgb_feature_optimize_v20_price_pred_vs_true.png')
    plt.close()
    print("预测价格 vs 实际价格图已保存为 11.21_xgb_feature_optimize_v20_price_pred_vs_true.png")

    # 保存预测结果
    submission = pd.DataFrame({'SaleID': test_saleid, 'price': test_pred})
    submission.to_csv('11.21_xgb_feature_optimize_v20_predict.csv', index=False)
    print(f"预测结果已保存为 11.21_xgb_feature_optimize_v20_predict.csv")
    ...
    plt.xlabel('重要性')
    plt.ylabel('特征')
    plt.savefig('11.21_xgb_feature_optimize_v20_importance_top30.png')
    plt.close()
    print("特征重要性Top30图已保存为 11.21_xgb_feature_optimize_v20_importance_top30.png")

    print("XGBoost特征工程优化v20 完成。")

"""
11.11_xgb_feature_optimize_v10.py

基于11.10版本，引入对regionCode的交叉验证目标编码，并为model和brand的目标编码增加平滑。
"""
import pandas as pd
import numpy as np
import matplotlib
...
def preprocess_and_feature_engineer(train_df, test_df):
    ...
    target_encode_cols = ['brand', 'model', 'regionCode'] # 新增 regionCode
    kf = KFold(n_splits=5, shuffle=True, random_state=42)
    
    # 平滑参数
    min_samples_leaf = 100 # 最小叶子节点数，低于此数量的类别将更多地被全局均值平滑
    smoothing = 10 # 平滑强度

    global_overall_mean = train_df['price'].mean() # 全局目标均值，用于平滑或新类别填充
    ...
    for col in target_encode_cols:
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(train_temp_df)):
            # 在训练折上计算目标均值
            stats = y_train_temp.iloc[train_idx].groupby(train_temp_df.iloc[train_idx][col]).agg(['count', 'mean'])
            # 应用平滑
            smooth_mean = (stats['count'] * stats['mean'] + global_overall_mean * smoothing) / (stats['count'] + smoothing)
            # 将平滑后的均值映射到验证折上
            train_temp_df.loc[train_temp_df.iloc[val_idx].index, col_encoded_name] = train_temp_df.iloc[val_idx][col].map(smooth_mean)
        
        # 将处理后的训练集目标编码结果更新回combined_df
        combined_df.loc[train_indices, col_encoded_name] = train_temp_df[col_encoded_name]

        # 对整个combined_df（包括测试集）使用全量训练集（原始train_df）的目标均值进行编码
        # 这里计算的是基于原始train_df的全量统计信息，用于测试集和最终训练集的填充
        global_stats = y_train_temp.groupby(train_df[col]).agg(['count', 'mean'])
        global_smooth_mean = (global_stats['count'] * global_stats['mean'] + global_overall_mean * smoothing) / (global_stats['count'] + smoothing)
        
        # 填充combined_df中的目标编码列（测试集和训练集中的NaN，即新类别或未处理部分）
        combined_df[col_encoded_name] = combined_df[col].map(global_smooth_mean)
        combined_df[col_encoded_name].fillna(global_overall_mean, inplace=True)

        print(f"已对特征 '{col}' 进行交叉验证目标编码 (含平滑)。")
    ...
    for col in X.columns:
        if X[col].dtype == 'object': 
            try:
                # 排除原始的'model', 'brand', 'regionCode'列（如果它们还在的话），因为它们应该被编码后的特征替代
    ...
    # 最终特征列表：移除原始的'model', 'brand', 'regionCode'列，并包含新的目标编码特征
    final_features = [col for col in X.columns if col not in ['price', 'SaleID', 'model', 'brand', 'regionCode']]
...
def train_and_evaluate_model(X, y, X_test, test_saleid):
    ...
    plt.xlabel('实际价格')
    plt.ylabel('预测价格')
    plt.savefig('11.11_xgb_feature_optimize_v10_price_pred_vs_true.png')
    plt.close()
    print("预测价格 vs 实际价格图已保存为 11.11_xgb_feature_optimize_v10_price_pred_vs_true.png")

    # 保存预测结果
    submission = pd.DataFrame({'SaleID': test_saleid, 'price': test_pred})
    submission.to_csv('11.11_xgb_feature_optimize_v10_predict.csv', index=False)
    print(f"预测结果已保存为 11.11_xgb_feature_optimize_v10_predict.csv")
    ...
    plt.xlabel('重要性')
    plt.ylabel('特征')
    plt.savefig('11.11_xgb_feature_optimize_v10_importance_top30.png')
    plt.close()
    print("特征重要性Top30图已保存为 11.11_xgb_feature_optimize_v10_importance_top30.png")

    print("XGBoost特征工程优化v10 完成。")

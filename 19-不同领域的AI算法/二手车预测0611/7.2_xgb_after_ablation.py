'''
基于特征消融实验推荐的68个最优特征组合，XGBoost单模型二手车预测脚本
迭代2000轮
XGBoost训练完成。
验证集 MAE: 534.1615
验证集 RMSE: 1194.2353
...
def main():
    ...
    result.to_csv('7.2_xgb_after_ablation_predict.csv', index=False)
    print('测试集预测结果已保存到 7.2_xgb_after_ablation_predict.csv')
    ...
    plt.xlabel('特征重要性')
    plt.title('7.2_xgb_after_ablation_XGBoost特征重要性Top30')
    plt.tight_layout()
    plt.savefig('7.2_xgb_after_ablation_importance_top30.png', dpi=150)
    plt.close()
    print('特征重要性Top30图已保存为 7.2_xgb_after_ablation_importance_top30.png')

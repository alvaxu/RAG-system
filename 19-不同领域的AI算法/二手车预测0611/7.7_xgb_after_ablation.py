...
7.5的基础上
迭代4000轮，不早停
[3999]  validation_0-rmse:0.22256
XGBoost训练完成。
验证集 MAE: 510.8480
验证集 RMSE: 1179.6630
...
def main():
    ...
    result.to_csv('7.7_xgb_after_ablation_predict.csv', index=False)
    print('测试集预测结果已保存到 7.7_xgb_after_ablation_predict.csv')
    ...
    plt.xlabel('特征重要性')
    plt.title('7.7_xgb_after_ablation_XGBoost特征重要性Top30')
    plt.tight_layout()
    plt.savefig('7.7_xgb_after_ablation_importance_top30.png', dpi=150)
    plt.close()
    print('特征重要性Top30图已保存为 7.7_xgb_after_ablation_importance_top30.png')

...
模型调优后的超参数:
  ...
  random_state: 42
  n_jobs: -1
预测结果已保存到 5.5_xgb_optimize_feature_predict.csv
...
def main():
    ...
    # 4. 保存模型
    joblib.dump(xgb, '5.5_xgb_optimize_feature_model.joblib')
    print('模型已保存到5.5_xgb_optimize_feature_model.joblib')
    ...
    result.to_csv('5.5_xgb_optimize_feature_predict.csv', index=False)
    print('预测结果已保存到 5.5_xgb_optimize_feature_predict.csv')
    ...
    plt.tight_layout()
    plt.savefig('5.5_xgb_optimize_feature_price_pred_vs_true.png', dpi=150)
    plt.close()
    print('预测价格与实际价格对比图已保存为 5.5_xgb_optimize_feature_price_pred_vs_true.png')
    ...
    plt.xlabel('特征重要性')
    plt.title('5.5_xgb_optimize_feature_XGBoost特征重要性Top50')
    plt.tight_layout()
    plt.savefig('5.5_xgb_optimize_feature_importance_top50.png', dpi=150)
    plt.close()
    print('特征重要性Top50图已保存为 5.5_xgb_optimize_feature_importance_top50.png.png')

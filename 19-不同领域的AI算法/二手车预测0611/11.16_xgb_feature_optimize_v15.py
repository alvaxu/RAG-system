"""
11.16_xgb_feature_optimize_v15.py

基于11.15版本，移除bodyType_missing特征，以诊断其异常重要性问题，并更新所有输出文件名为11.16_。
"""
import pandas as pd
import numpy as np
import matplotlib
...
def train_and_evaluate_model(X, y, X_test, test_saleid):
    ...
    plt.xlabel('实际价格')
    plt.ylabel('预测价格')
    plt.savefig('11.16_xgb_feature_optimize_v15_price_pred_vs_true.png')
    plt.close()
    print("预测价格 vs 实际价格图已保存为 11.16_xgb_feature_optimize_v15_price_pred_vs_true.png")

    # 保存预测结果
    submission = pd.DataFrame({'SaleID': test_saleid, 'price': test_pred})
    submission.to_csv('11.16_xgb_feature_optimize_v15_predict.csv', index=False)
    print(f"预测结果已保存为 11.16_xgb_feature_optimize_v15_predict.csv")
    ...
    plt.xlabel('重要性')
    plt.ylabel('特征')
    plt.savefig('11.16_xgb_feature_optimize_v15_importance_top30.png')
    plt.close()
    print("特征重要性Top30图已保存为 11.16_xgb_feature_optimize_v15_importance_top30.png")

    print("XGBoost特征工程优化v15 完成。")

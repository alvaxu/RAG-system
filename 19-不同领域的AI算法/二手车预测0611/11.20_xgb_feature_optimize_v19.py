"""
11.20_xgb_feature_optimize_v19.py

基于11.19版本，移除导致性能下降的v_3和v_12的多项式特征以及v_3和kilometer_log的交互特征，回到v17的稳定状态。
"""
import pandas as pd
import numpy as np
import matplotlib
...
def preprocess_and_feature_engineer(train_df, test_df):
    ...
    # --- 移除V18中引入的导致性能下降的特征 ---
    if 'v_3_poly2' in combined_df.columns:
        combined_df = combined_df.drop(columns=['v_3_poly2', 'v_3_poly3'])
        print("已移除特征 'v_3_poly2' 和 'v_3_poly3'.")
    if 'v_12_poly2' in combined_df.columns:
        combined_df = combined_df.drop(columns=['v_12_poly2', 'v_12_poly3'])
        print("已移除特征 'v_12_poly2' 和 'v_12_poly3'.")
    if 'v_3_kilometer_log_inter' in combined_df.columns:
        combined_df = combined_df.drop(columns=['v_3_kilometer_log_inter'])
        print("已移除特征 'v_3_kilometer_log_inter'.")
...
def train_and_evaluate_model(X, y, X_test, test_saleid):
    ...
    plt.xlabel('实际价格')
    plt.ylabel('预测价格')
    plt.savefig('11.20_xgb_feature_optimize_v19_price_pred_vs_true.png')
    plt.close()
    print("预测价格 vs 实际价格图已保存为 11.20_xgb_feature_optimize_v19_price_pred_vs_true.png")

    # 保存预测结果
    submission = pd.DataFrame({'SaleID': test_saleid, 'price': test_pred})
    submission.to_csv('11.20_xgb_feature_optimize_v19_predict.csv', index=False)
    print(f"预测结果已保存为 11.20_xgb_feature_optimize_v19_predict.csv")
    ...
    plt.xlabel('重要性')
    plt.ylabel('特征')
    plt.savefig('11.20_xgb_feature_optimize_v19_importance_top30.png')
    plt.close()
    print("特征重要性Top30图已保存为 11.20_xgb_feature_optimize_v19_importance_top30.png")

    print("XGBoost特征工程优化v19 完成。")

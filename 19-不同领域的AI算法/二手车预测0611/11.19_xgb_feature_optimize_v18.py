"""
11.19_xgb_feature_optimize_v18.py

基于11.18版本，探索v_3和v_12的多项式特征以及v_3和kilometer_log的交互特征，以期提升模型性能。
"""
import pandas as pd
import numpy as np
import matplotlib
...
def preprocess_and_feature_engineer(train_df, test_df):
    ...
    # --- 新增交互特征和时间相关特征 (V18中新增) ---
    # 10. v_3 的多项式特征
    if 'v_3' in combined_df.columns:
        combined_df['v_3_poly2'] = combined_df['v_3']**2
        combined_df['v_3_poly3'] = combined_df['v_3']**3
        print("已添加特征 'v_3_poly2' 和 'v_3_poly3'.")
    else:
        print("警告: 无法添加 'v_3' 的多项式特征，因为 'v_3' 列不存在。")

    # 11. v_12 的多项式特征
    if 'v_12' in combined_df.columns:
        combined_df['v_12_poly2'] = combined_df['v_12']**2
        combined_df['v_12_poly3'] = combined_df['v_12']**3
        print("已添加特征 'v_12_poly2' 和 'v_12_poly3'.")
    else:
        print("警告: 无法添加 'v_12' 的多项式特征，因为 'v_12' 列不存在。")

    # 12. v_3 * kilometer_log 交互特征
    if 'v_3' in combined_df.columns and 'kilometer_log' in combined_df.columns:
        combined_df['v_3_kilometer_log_inter'] = combined_df['v_3'] * combined_df['kilometer_log']
        print("已添加特征 'v_3_kilometer_log_inter'.")
    else:
        print("警告: 无法添加 'v_3_kilometer_log_inter'，因为 'v_3' 或 'kilometer_log' 列不存在。")
    ...
    # 确保V18新增特征在最终特征列表中
    if 'v_3_poly2' in combined_df.columns and 'v_3_poly2' not in final_features_to_select:
        final_features_to_select.append('v_3_poly2')
    if 'v_3_poly3' in combined_df.columns and 'v_3_poly3' not in final_features_to_select:
        final_features_to_select.append('v_3_poly3')
    if 'v_12_poly2' in combined_df.columns and 'v_12_poly2' not in final_features_to_select:
        final_features_to_select.append('v_12_poly2')
    if 'v_12_poly3' in combined_df.columns and 'v_12_poly3' not in final_features_to_select:
        final_features_to_select.append('v_12_poly3')
    if 'v_3_kilometer_log_inter' in combined_df.columns and 'v_3_kilometer_log_inter' not in final_features_to_select:
        final_features_to_select.append('v_3_kilometer_log_inter')
...
def train_and_evaluate_model(X, y, X_test, test_saleid):
    ...
    plt.xlabel('实际价格')
    plt.ylabel('预测价格')
    plt.savefig('11.19_xgb_feature_optimize_v18_price_pred_vs_true.png')
    plt.close()
    print("预测价格 vs 实际价格图已保存为 11.19_xgb_feature_optimize_v18_price_pred_vs_true.png")

    # 保存预测结果
    submission = pd.DataFrame({'SaleID': test_saleid, 'price': test_pred})
    submission.to_csv('11.19_xgb_feature_optimize_v18_predict.csv', index=False)
    print(f"预测结果已保存为 11.19_xgb_feature_optimize_v18_predict.csv")
    ...
    plt.xlabel('重要性')
    plt.ylabel('特征')
    plt.savefig('11.19_xgb_feature_optimize_v18_importance_top30.png')
    plt.close()
    print("特征重要性Top30图已保存为 11.19_xgb_feature_optimize_v18_importance_top30.png")

    print("XGBoost特征工程优化v18 完成。")

...
"""
:function: 只用特征重要性Top50特征，优化XGBoost超参数，缓解过拟合，输出相关结果
:param 无
:return: 无
"""
def main():
    ...
    # ========== 只保留特征重要性Top50的特征 ==========
    top50_features = [
        'v3_car_age_interaction', 'v12_power_log_interaction', 'v_3', 'v_10',
        'notRepairedDamage', 'v_pca_0', 'region_brand_price_mean', 'km_bin',
        'regYear', 'km_age', 'brand_price_count', 'v_pca_1', 'v_pca_2', 'power',
        'car_age', 'region_brand_price_std', 'power_log', 'age_segment', 'brand_price_std',
        'power_bin', 'brand_v3_mean', 'v_pca_4', 'regSeason_missing', 'v_pca_3',
        'brand_v3_std', 'km_per_year', 'regYear_bin', 'bodyType', 'brand_model_age_freq',
        'v_pca_5', 'power_age', 'powertrain_type_freq', 'v_pca_8', 'v_pca_9', 'v_pca_7',
        'v_pca_6', 'fuelType_missing', 'creatMonth', 'brand_price_mean', 'gearbox_missing',
        'regMonth', 'fuelType', 'brand', 'regSeason', 'brand_price_ratio', 'brand_age_segment_freq',
        'gearbox', 'brand_power_bin_freq'
    ]
    features = [f for f in top50_features if f in data.columns]
    ...
    xgb = XGBRegressor(
        max_depth=5,              # 降低树深度，缓解过拟合
        learning_rate=0.02,       # 学习率适当降低
        n_estimators=800,         # 迭代次数减少
        subsample=0.7,            # 行采样比例
    ...
    xgb.fit(X_train, np.log1p(y_train), eval_set=[(X_val, np.log1p(y_val))], early_stopping_rounds=30, verbose=50)
    end_time = time.time()
    print(f"XGBoost模型训练结束，结束时间: {time.ctime(end_time)}\n")
    print(f"总训练时长: {end_time - start_time:.2f}秒")

    # ========== 保存模型 ==========
    joblib.dump(xgb, '5.3_xgb_optimize_feature_top50_model.joblib')
    print('模型已保存到 5.3_xgb_optimize_feature_top50_model.joblib')
    ...
    result.to_csv('5.3_xgb_optimize_feature_top50_predict.csv', index=False)
    print('预测结果已保存到 5.3_xgb_optimize_feature_top50_predict.csv')
    ...
    plt.tight_layout()
    plt.savefig('5.3_xgb_optimize_feature_top50_price_pred_vs_true.png', dpi=150)
    plt.close()
    print('预测价格与实际价格对比图已保存为 5.3_xgb_optimize_feature_top50_price_pred_vs_true.png')

    # ========== 绘制特征重要性top50 ==========
    importances = xgb.feature_importances_
    feat_names = X.columns
    n_feats = min(50, len(importances))
    indices = np.argsort(importances)[::-1][:n_feats]
    plt.figure(figsize=(10,6))
    plt.barh(range(n_feats), importances[indices][::-1], align='center')
    plt.yticks(range(n_feats), [feat_names[i] for i in indices][::-1])
    plt.xlabel('特征重要性')
    plt.title('5.3_xgb_optimize_feature_top50_XGBoost特征重要性Top50')
    plt.tight_layout()
    plt.savefig('5.3_xgb_optimize_feature_top50_importance_1.png', dpi=150)
    plt.close()
    print('特征重要性Top50图已保存为 5.3_xgb_optimize_feature_top50_importance_1.png')

'''
7.3的基础上增加迭代次数6000,早停轮次100轮,基于MAE早停
XGBoost训练完成。
Blending验证集 MAE: 510.3736
Blending验证集 MAE: 510.3736
Blending验证集 RMSE: 1159.2563
blending权重: [0.58946261 0.19780528 0.22210439], 截距: 20.78602089970991
三模型blending权重分别为: XGBoost: 0.5895, CatBoost: 0.1978, LightGBM: 0.2221
XGBoost验证集 MAE: 508.5802, RMSE: 1179.1964
CatBoost验证集 MAE: 522.5206, RMSE: 1240.3085
LightGBM验证集 MAE: 554.4593, RMSE: 1214.1417
Blending测试集预测结果已保存到 7.6_blending_after_ablation_predict.csv
'''
...
def main():
    ...
    cat = CatBoostRegressor(
        iterations=6000, learning_rate=0.03, depth=10, subsample=0.8, colsample_bylevel=0.9,
        random_seed=42, verbose=100, loss_function='MAE', early_stopping_rounds=100
    )
    ...
    print("\n========== LightGBM模型训练 ==========")
    lgb = LGBMRegressor(
        n_estimators=6000, learning_rate=0.03, max_depth=10, subsample=0.8, colsample_bytree=0.9,
    ...
    lgb.fit(X_train_all, np.log1p(y_train_all), eval_set=[(X_blend, np.log1p(y_blend))], eval_metric='MAE',callbacks=callbacks)
    print("LightGBM训练完成。")
    
    print("\n========== XGBoost模型训练 ==========")
    xgb = XGBRegressor(
        max_depth=10, learning_rate=0.03, n_estimators=6000, subsample=0.8, colsample_bytree=0.9,
    ...
    result.to_csv('7.6_blending_after_ablation_predict.csv', index=False)
    print('Blending测试集预测结果已保存到 7.6_blending_after_ablation_predict.csv')
    ...
    plt.xlabel('特征重要性')
    plt.title('7.6_blending_after_ablation_XGBoost特征重要性Top30')
    plt.tight_layout()
    plt.savefig('7.6_blending_after_ablation_importance_top30.png', dpi=150)
    plt.close()
    print('XGBoost特征重要性Top30图已保存为 7.6_blending_after_ablation_importance_top30.png')

...
def main():
    ...
    # 4.1 对齐特征列以匹配已加载模型
    print("\n========== 对齐特征列以匹配已加载模型 ==========")
    model_features = best_xgb_model.get_booster().feature_names
    
    # 处理 X_val
    # 找出 X_val 中多余的特征，并删除
    extra_features_val = [f for f in X_val.columns if f not in model_features]
    if extra_features_val:
        print(f"警告：以下特征在验证集X_val中多余，模型训练时不存在: {extra_features_val}")
        X_val = X_val.drop(columns=extra_features_val)

    # 找出模型期望但 X_val 中缺失的特征，并添加（填充0）
    missing_features_val = [f for f in model_features if f not in X_val.columns]
    if missing_features_val:
        print(f"警告：以下特征在验证集X_val中缺失，模型训练时存在: {missing_features_val}")
        for f in missing_features_val:
            X_val[f] = 0 # 缺失指示特征通常填充0

    # 确保 X_val 的特征顺序与模型训练时一致
    X_val = X_val[model_features]

    # 处理 X_test
    # 找出 X_test 中多余的特征，并删除
    extra_features_test = [f for f in X_test.columns if f not in model_features]
    if extra_features_test:
        print(f"警告：以下特征在测试集X_test中多余，模型训练时不存在: {extra_features_test}")
        X_test = X_test.drop(columns=extra_features_test)

    # 找出模型期望但 X_test 中缺失的特征，并添加（填充0）
    missing_features_test = [f for f in model_features if f not in X_test.columns]
    if missing_features_test:
        print(f"警告：以下特征在测试集X_test中缺失，模型训练时存在: {missing_features_test}")
        for f in missing_features_test:
            X_test[f] = 0 # 缺失指示特征通常填充0

    # 确保 X_test 的特征顺序与模型训练时一致
    ...
    test_pred_log = best_xgb_model.predict(X_test)
    ...
    result.to_csv('xgb_save_train_result_gridsearch.csv', index=False)
    print('预测结果已保存到 xgb_save_train_result_gridsearch.csv')
    ...
    plt.tight_layout()
    plt.savefig('price_pred_vs_true_gridsearch.png', dpi=150)
    plt.close()
    print('预测价格与实际价格对比图已保存为 price_pred_vs_true_gridsearch.png')

    # ========== SHAP特征重要性分析 ========== 
    print("\n========== 开始SHAP特征重要性分析 ==========")
    start_time_shap = time.time()
    explainer = shap.TreeExplainer(best_xgb_model)
    shap_values = explainer.shap_values(X_train) # 在训练集上计算SHAP值
    ...
    plt.tight_layout()
    plt.savefig('shap_feature_importance_gridsearch.png', dpi=150)
    plt.close()
    print('SHAP特征重要性图已保存为 shap_feature_importance_gridsearch.png')

    print("========== 绘制SHAP Summary Plot (dot plot) ==========")
    plt.figure(figsize=(10, 7))
    shap.summary_plot(shap_values, X_train, show=False)
    plt.title('SHAP Summary Plot (Optimized Model)')
    plt.tight_layout()
    plt.savefig('shap_summary_plot_gridsearch.png', dpi=150)
    plt.close()
    print('SHAP Summary Plot已保存为 shap_summary_plot_gridsearch.png')

    print("优化程序执行完毕。")
    print("您可以根据'最佳参数组合'和'SHAP特征重要性图'来更新您的模型配置。")

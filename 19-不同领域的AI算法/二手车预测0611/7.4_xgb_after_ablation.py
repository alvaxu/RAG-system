'''
基于特征消融实验推荐的68个最优特征组合,再去除：'v_7', 'v_pca_5'三个有害特征，
XGBoost单模型二手车预测脚本
迭代2000轮
XGBoost训练完成。
验证集 MAE: 543.1087
...
BEST_FEATURES = [
    ...
    'v_13', 'model_freq', 'brand_price_median', 'fuelType_missing', 'v_pca_8', 'brand_km_bin', 'v_pca_6', 'name',
    'v_pca_7', 'v_4', 'regionCode', 'creatMonth', 'v_pca_9', 'brand', 'creatDay', 'regMonth', 'regDay', 'regSeason','model'
    # 已去除：'v_7', 'v_pca_5'
]
...
def main():
    ...
    result.to_csv('7.4_xgb_after_ablation_predict.csv', index=False)
    print('测试集预测结果已保存到 7.4_xgb_after_ablation_predict.csv')
    ...
    plt.xlabel('特征重要性')
    plt.title('7.4_xgb_after_ablation_XGBoost特征重要性Top30')
    plt.tight_layout()
    plt.savefig('7.4_xgb_after_ablation_importance_top30.png', dpi=150)
    plt.close()
    print('特征重要性Top30图已保存为 7.4_xgb_after_ablation_importance_top30.png')

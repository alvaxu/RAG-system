...
def main():
    ...
    xgb = XGBRegressor(
        max_depth=8, learning_rate=0.03, n_estimators=2000, subsample=0.8, colsample_bytree=0.9,
        random_state=42, n_jobs=-1, eval_metric='rmse'
    )
    xgb.fit(X_train, np.log1p(y_train), eval_set=[(X_val, np.log1p(y_val))], early_stopping_rounds=50, verbose=50)
    end_time = time.time() # 记录结束时间
    print(f"XGBoost模型训练结束，结束时间: {time.ctime(end_time)}\n") # 打印结束时间
    print(f"总训练时长: {end_time - start_time:.2f}秒") # 打印总时长
    # 4. 保存模型
    joblib.dump(xgb, '5.3_xgb_optimize_feature_model.joblib')
    print('模型已保存到5.3_xgb_optimize_feature_model.joblib')
    ...
    result.to_csv('5.3_xgb_optimize_feature_predict.csv', index=False)
    print('预测结果已保存到 5.3_xgb_optimize_feature_predict.csv')
    ...
    plt.tight_layout()
    plt.savefig('5.3_xgb_optimize_feature_price_pred_vs_true.png', dpi=150)
    plt.close()
    print('预测价格与实际价格对比图已保存为 5.3_xgb_optimize_feature_price_pred_vs_true.png')
    ...
    plt.xlabel('特征重要性')
    plt.title('5.3_xgb_optimize_feature_XGBoost特征重要性Top50')
    plt.tight_layout()
    plt.savefig('5.3_xgb_optimize_feature_importance_top50.png', dpi=150)
    plt.close()
    print('特征重要性Top50图已保存为 5.3_xgb_optimize_feature_importance_top50.png.png')

...
def main():
    ...
    # 设置回调函数（包含早停）
    callbacks = [
        lgbm.early_stopping(stopping_rounds=50),  # 早停轮数
        lgbm.log_evaluation(period=50)  # 每50轮输出日志
    ]
    lgb.fit(X_train_all, np.log1p(y_train_all), eval_set=[(X_blend, np.log1p(y_blend))], eval_metric='rmse',callbacks=callbacks)

    print("LightGBM训练完成。")
    joblib.dump(lgb, '6.0_blending_lgb_model.joblib')
    print('LightGBM模型已保存到 6.0_blending_lgb_model.joblib')
    ...
    print("XGBoost训练完成。")
    joblib.dump(xgb, '6.0_blending_xgb_model.joblib')
    print('XGBoost模型已保存到 6.0_blending_xgb_model.joblib')
    ...
    print("CatBoost训练完成。")
    joblib.dump(cat, '6.0_blending_cat_model.joblib')
    print('CatBoost模型已保存到 6.0_blending_cat_model.joblib')

    # # 5. LightGBM模型训练
    # print("\n========== LightGBM模型训练 ==========")
    # lgb = LGBMRegressor(
    #     n_estimators=4000, learning_rate=0.03, max_depth=10, subsample=0.8, colsample_bytree=0.9,
    #     random_state=42, n_jobs=-1, metric='rmse'
    # )
    # lgb.fit(X_train_all, np.log1p(y_train_all), eval_set=[(X_blend, np.log1p(y_blend))], early_stopping_rounds=50, verbose=50)
    # print("LightGBM训练完成。")
    # joblib.dump(lgb, '6.0_blending_lgb_model.joblib')
    # print('LightGBM模型已保存到 6.0_blending_lgb_model.joblib')
    ...
    result.to_csv('6.0_blending_predict.csv', index=False)
    print('Blending预测结果已保存到 6.0_blending_predict.csv')
    ...
    plt.xlabel('特征重要性')
    plt.title('6.0_blending_XGBoost特征重要性Top50')
    plt.tight_layout()
    plt.savefig('6.0_blending_importance_top50.png', dpi=150)
    plt.close()
    print('特征重要性Top50图已保存为 6.0_blending_importance_top50.png')

"""
11.13_xgb_feature_optimize_v12.py

基于11.12版本，完全移除brand和model的交叉验证目标编码，回到更精简的特征集。
保留v_0 * v_12, power_per_year, v_3_abs等有效特征。
"""
import pandas as pd
import numpy as np
import matplotlib
...
def preprocess_and_feature_engineer(train_df, test_df):
    ...
    # 7. 移除交叉验证目标编码
    # 之前尝试的brand, model, regionCode目标编码均未显著提升性能，故移除。
    print("已移除brand和model的交叉验证目标编码。")
...
def train_and_evaluate_model(X, y, X_test, test_saleid):
    ...
    plt.xlabel('实际价格')
    plt.ylabel('预测价格')
    plt.savefig('11.13_xgb_feature_optimize_v12_price_pred_vs_true.png')
    plt.close()
    print("预测价格 vs 实际价格图已保存为 11.13_xgb_feature_optimize_v12_price_pred_vs_true.png")

    # 保存预测结果
    submission = pd.DataFrame({'SaleID': test_saleid, 'price': test_pred})
    submission.to_csv('11.13_xgb_feature_optimize_v12_predict.csv', index=False)
    print(f"预测结果已保存为 11.13_xgb_feature_optimize_v12_predict.csv")
    ...
    plt.xlabel('重要性')
    plt.ylabel('特征')
    plt.savefig('11.13_xgb_feature_optimize_v12_importance_top30.png')
    plt.close()
    print("特征重要性Top30图已保存为 11.13_xgb_feature_optimize_v12_importance_top30.png")

    print("XGBoost特征工程优化v12 完成。")

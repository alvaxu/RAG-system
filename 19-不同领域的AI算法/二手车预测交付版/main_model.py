...
[1999]  validation_0-mae:0.11511
XGBoost模型重新训练完成。
重新训练模型在验证集上的 MAE (xgb.best_score): 0.1151
验证集 MAE: 511.3080
验证集 RMSE: 1218.5332
成绩：494.2830
...
def train_and_evaluate_model(X, y, X_test, test_saleid):
    ...
    plt.xlabel('实际价格')
    plt.ylabel('预测价格')
    plt.savefig('main_model_price_pred_vs_true.png')
    plt.close()
    print("预测价格 vs 实际价格图已保存为 main_model_price_pred_vs_true.png")

    # 保存预测结果
    submission = pd.DataFrame({'SaleID': test_saleid, 'price': test_pred})
    submission.to_csv('main_model_predict.csv', index=False)
    print(f"预测结果已保存为 main_model_predict.csv")

    # 保存验证集预测
    val_pred_df = pd.DataFrame({'SaleID': X_val['SaleID'] if 'SaleID' in X_val.columns else np.arange(len(X_val)), 'price': val_pred})
    val_pred_df.to_csv('main_model_val_pred.csv', index=False)
    ...
    plt.xlabel('重要性')
    plt.ylabel('特征')
    plt.savefig('main_model_importance_top30.png')
    plt.close()
    print("特征重要性Top30图已保存为 main_model_importance_top30.png")

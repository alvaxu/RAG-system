...
def main():
    ...
    # 2. 使用K折交叉验证进行训练
    kf = KFold(n_splits=5, shuffle=True, random_state=42) # 设置KFold参数
    oof_preds = np.zeros(X.shape[0]) # 存储out-of-fold预测
    test_preds = np.zeros(X_test.shape[0]) # 存储测试集预测
    feature_importances = pd.DataFrame(index=X.columns) # 存储特征重要性

    start_time = time.time() # 记录开始时间
    print(f"LGBM模型交叉验证训练开始，开始时间: {time.ctime(start_time)}") # 打印开始时间
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(X, y)):
        ...
        lgbm.fit(X_train, np.log1p(y_train),
                 ...
                 callbacks=[lgb.early_stopping(stopping_rounds=50, verbose=False)]) # 添加早停回调函数

        oof_preds[val_index] = np.expm1(lgbm.predict(X_val)) # 对验证集进行预测，并expm1转换
        test_preds += np.expm1(lgbm.predict(X_test)) / kf.n_splits # 对测试集进行预测，并累加

        # 记录当前折叠的特征重要性
        feature_importances[f'fold_{fold+1}'] = lgbm.feature_importances_

    end_time = time.time() # 记录结束时间
    print(f"LGBM模型交叉验证训练结束，结束时间: {time.ctime(end_time)}") # 打印结束时间
    ...
    # 4. 保存最终模型（可选：保存每个折叠的模型或保存基于oof数据的模型）
    # joblib.dump(lgbm, 'lgbm_model_cv_final.joblib') # 可以选择不保存单一模型，或保存最后一个折叠的模型
    # print('模型已保存到 lgbm_model_cv_final.joblib')

    # 5. 保存预测结果
    result = pd.DataFrame({'SaleID': test_saleid, 'price': test_preds}) if test_saleid is not None else pd.DataFrame({'price': test_preds})
    result.to_csv('used_car_sample_submit_lgbm_cv_final.csv', index=False) # 修改文件名以区分交叉验证
    print('预测结果已保存到 used_car_sample_submit_lgbm_cv_final.csv')

    # 6. 评估模型（MAE、RMSE）- 使用out-of-fold预测
    mae = mean_absolute_error(y, oof_preds)
    rmse = np.sqrt(mean_squared_error(y, oof_preds))
    print(f'交叉验证 OOF MAE: {mae:.4f}, OOF RMSE: {rmse:.4f}')

    # 7. 绘制预测价格和实际价格对比图（使用out-of-fold预测）
    plt.figure(figsize=(8,6))
    plt.scatter(y, oof_preds, alpha=0.3)
    plt.plot([y.min(), y.max()], [y.min(), y.max()], 'r--', lw=2)
    plt.xlabel('实际价格')
    plt.ylabel('预测价格')
    plt.title('LGBM交叉验证 OOF预测价格 vs 实际价格') # 修改标题
    plt.tight_layout()
    plt.savefig('price_pred_vs_true_lgbm_cv.png', dpi=150) # 修改文件名
    plt.close()
    print('LGBM交叉验证 OOF预测价格与实际价格对比图已保存为 price_pred_vs_true_lgbm_cv.png') # 修改打印信息

    # 8. 绘制特征重要性top20
    plt.figure(figsize=(10,6))
    top20_importances = feature_importances['average'].head(20)[::-1] # 取平均重要性最高的20个
    plt.barh(range(20), top20_importances.values, align='center')
    plt.yticks(range(20), top20_importances.index.tolist())
    plt.xlabel('特征重要性 (平均)')
    plt.title('LGBM交叉验证 特征重要性Top20') # 修改标题
    plt.tight_layout()
    plt.savefig('lgbm_feature_importance_top20_cv.png', dpi=150) # 修改文件名
    plt.close()
    print('LGBM交叉验证 特征重要性Top20图已保存为 lgbm_feature_importance_top20_cv.png') # 修改打印信息

...
def main():
    ...
    # 4. 保存模型
    joblib.dump(xgb, 'xgb_model_final_optimize.joblib')
    print('模型已保存到 xgb_model_final_optimize.joblib')
    ...
    result.to_csv('xgb_save_train_result_optimize_1.csv', index=False)
    print('预测结果已保存到 xgb_save_train_result_optimize_1.csv')
    ...
    plt.tight_layout()
    plt.savefig('price_pred_vs_true_optimize_1.png', dpi=150)
    plt.close()
    print('预测价格与实际价格对比图已保存为 price_pred_vs_true_optimize_1.png')
    ...
    plt.tight_layout()
    plt.savefig('xgb_feature_importance_top20_optimize_1.png', dpi=150)
    plt.close()
    print('特征重要性Top20图已保存为 xgb_feature_importance_top20_optimize_1.png')

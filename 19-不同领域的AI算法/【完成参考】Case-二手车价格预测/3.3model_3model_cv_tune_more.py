...
def main():
    ...
    # 2. Stacking集成模型
    xgb = XGBRegressor(
        max_depth=10, learning_rate=0.03, n_estimators=1000, subsample=0.8, colsample_bytree=0.9,
    ...
    cat = CatBoostRegressor(
        iterations=1000, depth=10, learning_rate=0.03, subsample=0.8, random_state=42, verbose=0,
        l2_leaf_reg=3.0
    )
    ...
    print(f'Stacking 5折CV MAE均值: {np.mean(maes):.4f}, RMSE均值: {np.mean(rmses):.4f}')

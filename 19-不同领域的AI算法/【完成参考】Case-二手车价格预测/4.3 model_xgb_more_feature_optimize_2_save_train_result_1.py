...
def main():
    ...
    # 提前去除常量特征
    # 检查seller和offerType是否为常量
    if data['seller'].nunique() == 1:
        print(f"检测到特征 'seller' 为常量，已删除。")
        data = data.drop('seller', axis=1)
    if data['offerType'].nunique() == 1:
        print(f"检测到特征 'offerType' 为常量，已删除。")
    ...
    print('训练集品牌v3统计特征已保存到 train_brand_v3_stats.joblib')
    ...
    # 根据用户要求，从高相关性删除列表中移除特定特征，以确保它们被保留
    # 这些特征是 regYear, v_6, brand_price_ratio
    features_to_retain = ['regYear', 'v_6', 'brand_price_ratio']
    ...
    if len(to_drop_corr) > 0:
        ...
        print('高相关性特征已删除(阈值 > 0.99)，并确保保留特定特征。')
    else:
        print('未发现高相关性特征 (阈值 > 0.99)。')
    ...
    xgb = XGBRegressor(
        max_depth=10, learning_rate=0.02, n_estimators=4000, subsample=0.8, colsample_bytree=0.9,
    ...
    # 4. 保存模型
    joblib.dump(xgb, 'xgb_model_final_optimize_21.joblib')
    print('模型已保存到 xgb_model_final_optimize_21.joblib')
    ...
    result.to_csv('xgb_save_train_result_optimize_21.csv', index=False)
    print('预测结果已保存到 xgb_save_train_result_optimize_21.csv')
    ...
    plt.tight_layout()
    plt.savefig('price_pred_vs_true_optimize_21.png', dpi=150)
    plt.close()
    print('预测价格与实际价格对比图已保存为 price_pred_vs_true_optimize_21.png')
    ...
    plt.tight_layout()
    plt.savefig('xgb_feature_importance_top20_optimize_21.png', dpi=150)
    plt.close()
    print('特征重要性Top20图已保存为 xgb_feature_importance_top20_optimize_21.png')

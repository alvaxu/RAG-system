#!/usr/bin/env python
# -*- coding: utf-8 -*-
# 完全模仿 fund_qa_langgraph.py 的ReAct风格，重写了 hybrid_wealth_advisor_langgraph.py 文件
# 中的 reactive_processing 函数及其相关Prompt和Parser，确保该函数在用户询问有关上证指数的时候，
# 可以调用工具query_shanghai_index
# 具体如下：
"""
...
REACT_AGENT_TMPL = """你是一个财富管理投顾AI助手，请根据用户的问题选择合适的工具来回答。
...
# ========== 重写reactive_processing ===========
def reactive_processing(state: WealthAdvisorState) -> WealthAdvisorState:
    print("[DEBUG] 进入节点: reactive_processing (ReAct工具链自定义Prompt/Parser版)")
    try:
        tools = [
            Tool(
                name="上证指数查询",

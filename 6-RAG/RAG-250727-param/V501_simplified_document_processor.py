...
class SimplifiedDocumentProcessor:
    ...
    def process_from_markdown(self, md_dir: str = None, vector_db_path: str = None) -> bool:
        ...
        try:
            # 使用配置中的路径，如果参数提供则覆盖
            md_dir = md_dir or self.config.md_dir
            vector_db_path = vector_db_path or self.config.get_vector_db_path()
...
def main():
    ...
    """
    # 创建参数解析器
    parser = argparse.ArgumentParser(description='简化版文档处理器')
    ...
    try:
        ...
        if args.show_config:
            print("配置信息:")
            print(f"  - PDF目录: {processor.config.pdf_dir}")
            print(f"  - Markdown目录: {processor.config.md_dir}")
            print(f"  - 输出目录: {processor.config.output_dir}")
        ...
        if args.validate:
            print("\n配置验证结果:")
            # 检查必需的路径
            required_paths = ['pdf_dir', 'md_dir', 'output_dir', 'vector_db_dir', 'central_images_dir']

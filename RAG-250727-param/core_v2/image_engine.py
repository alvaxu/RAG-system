...
class ImageEngine(BaseEngine):
    ...
    def _validate_config(self):
        """验证图片引擎配置"""
        if not isinstance(self.config, ImageEngineConfig):
            raise ValueError("配置必须是ImageEngineConfig类型")
        
        if self.config.image_similarity_threshold < 0 or self.config.image_similarity_threshold > 1:
    ...
    def _search_images(self, query: str, intent: Dict[str, Any], **kwargs) -> List[Any]:
        ...
        if intent['figure_numbers']:
            for doc_id, doc in self.image_docs.items():
                ...
                if any(f'图{num}' in caption for num in intent['figure_numbers']):
                    results.append({
                        'doc_id': doc_id,
                        'doc': doc,
                        'score': 1.0,
        ...
        if not results and self.config.enable_fuzzy_match:
            results = self._fuzzy_image_search(query, intent)
    ...
    def _calculate_keyword_match(self, doc: Any, keywords: List[str]) -> float:
        ...
        # 获取所有文本字段
        text_fields = [
            doc.metadata.get('img_caption', ''),
            doc.metadata.get('image_title', ''),
            doc.metadata.get('enhanced_description', '')
        ]

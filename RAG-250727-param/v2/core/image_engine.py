...
class ImageEngine(BaseEngine):
    ...
    def _enhance_reranked_results(self, reranked_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        增强reranked_results：使用明确的字段映射关系
        ...
        for result in reranked_results:
            if isinstance(result, dict) and 'doc' in result:
                ...
                if hasattr(doc, 'metadata') and doc.metadata:
                    ...
                    # 2. 图片字段 - 明确的对应关系
                    enhanced_result.update({
                        'image_path': metadata.get('image_path', ''),                    # 明确从image_path获取
                        'caption': metadata.get('img_caption', ['无标题']),             # 明确从img_caption获取
                        'footnote': metadata.get('img_footnote', []),                   # 明确从img_footnote获取
                        'enhanced_description': metadata.get('enhanced_description', ''), # 明确从enhanced_description获取
                        ...
                        'semantic_features': metadata.get('semantic_features', {}),     # 明确从semantic_features获取
                        'enhancement_enabled': metadata.get('enhancement_enabled', False), # 明确从enhancement_enabled获取
                        'enhancement_timestamp': metadata.get('enhancement_timestamp', '') # 明确从enhancement_timestamp获取
                    })
                    ...
                    if not llm_context_parts and metadata.get('img_caption'):
                        llm_context_parts.append(' '.join(metadata['img_caption']))

...
class TableEngine(BaseEngine):
    ...
    def _load_documents(self):
        ...
        try:
            if self.document_loader:
                ...
                logger.warning("未提供文档加载器或向量数据库，表格引擎将无法工作")
                return
                
            self._docs_loaded = True
            logger.info(f"表格引擎加载了 {len(self.table_docs)} 个表格文档")
            
        except Exception as e:
            logger.error(f"加载表格文档失败: {e}")
            self._docs_loaded = False
    ...
    def process_query(self, query: str, **kwargs) -> QueryResult:
        ...
        try:
            ...
            return QueryResult(
                ...
                engine_name=self.name,
                metadata={'total_tables': len(self.table_docs)}
            )
    ...
    def _search_tables_with_lower_threshold(self, query: str, threshold: float) -> List[Dict[str, Any]]:
        ...
        """
        # 临时降低阈值
        original_threshold = getattr(self.config, 'table_similarity_threshold', 0.7)

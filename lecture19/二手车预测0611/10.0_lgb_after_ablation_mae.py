...
7.9 的基础上用
训练集85%，验证集15%
[2500]  valid_0's l1: 0.125152  valid_0's l2: 0.050699
Early stopping, best iteration is:
[2420]  valid_0's l1: 0.125292  valid_0's l2: 0.0506901
LightGBM训练完成。
LightGBM模型已保存为 10.0_LightGBM_model.txt
验证集 MAE: 575.0268
验证集 RMSE: 1260.6541
...
def main():
    ...
    lgb.fit(X_train, np.log1p(y_train), eval_set=(X_val, np.log1p(y_val)), eval_metric='MAE',callbacks=callbacks)
  
    print("LightGBM训练完成。")
   

    # 保存模型到本地文件
    # :function: 保存训练好的LightGBM模型，便于后续加载和复用
    # :param lgb: 训练好的LGBMRegressor模型
    # :return: 无
    lgb.booster_.save_model('10.0_LightGBM_model.txt')
    print('LightGBM模型已保存为 10.0_LightGBM_model.txt')

    # 5. 验证集评估
    val_pred = np.expm1(lgb.predict(X_val))
    ...
    result.to_csv('10.0_LightGBM_after_ablation_mae_predict.csv', index=False)
    print('测试集预测结果已保存到 10.0_LightGBM_after_ablation_mae_predict.csv')
    ...
    plt.xlabel('特征重要性')
    plt.title('10.0_LightGBM_after_ablation_mae_LightGBM特征重要性Top30')
    plt.tight_layout()
    plt.savefig('10.0_LightGBM_after_ablation_mae_importance_top30.png', dpi=150)
    plt.close()
    print('特征重要性Top30图已保存为 10.0_LightGBM_after_ablation_mae_importance_top30.png')

...
def main():
    ...
    result.to_csv('7.8_xgb_after_ablation_mae_predict.csv', index=False)
    print('测试集预测结果已保存到 7.8_xgb_after_ablation_mae_predict.csv')
    ...
    plt.xlabel('特征重要性')
    plt.title('7.8_xgb_after_ablation_mae_XGBoost特征重要性Top30')
    plt.tight_layout()
    plt.savefig('7.8_xgb_after_ablation_mae_importance_top30.png', dpi=150)
    plt.close()
    print('特征重要性Top30图已保存为 7.8_xgb_after_ablation_mae_importance_top30.png')

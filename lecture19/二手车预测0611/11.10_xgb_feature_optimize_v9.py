"""
11.10_xgb_feature_optimize_v9.py

基于11.9版本，移除冗余的model和brand类别特征编码，引入交叉验证目标编码，并更新输出文件名。
"""
import pandas as pd
import numpy as np
import matplotlib
...
from sklearn.model_selection import train_test_split, KFold # 引入KFold
...
def preprocess_and_feature_engineer(train_df, test_df):
    ...
    # 7. 交叉验证目标编码 (Target Encoding)
    # 仅对训练集进行目标编码，然后将训练集编码的映射应用到测试集
    # 为了避免数据泄露，使用K折交叉验证进行编码
    target_encode_cols = ['brand', 'model']
    kf = KFold(n_splits=5, shuffle=True, random_state=42)
    
    # 确保combined_df中包含'price'列用于目标编码（只对训练集部分有效）
    # 先处理训练集部分
    train_combined_df = combined_df[combined_df['is_train'] == 1].copy()
    y_train_temp = train_df['price'].copy() # 获取原始训练集的y
    
    for col in target_encode_cols:
        col_encoded_name = f'{col}_target_encoded'
        train_combined_df[col_encoded_name] = np.nan
        
        # 强制转换为字符串类型，以防万一
        train_combined_df[col] = train_combined_df[col].astype(str)
        combined_df[col] = combined_df[col].astype(str) # 确保整个combined_df也是字符串

        for fold, (train_idx, val_idx) in enumerate(kf.split(train_combined_df)):
            # 在训练折上计算目标均值
            mean_target = y_train_temp.iloc[train_idx].groupby(train_combined_df.iloc[train_idx][col]).mean()
            # 将均值映射到验证折上
            train_combined_df.loc[train_combined_df.iloc[val_idx].index, col_encoded_name] = train_combined_df.iloc[val_idx][col].map(mean_target)

        # 对测试集和整个训练集（用于最终训练）使用全量训练集的目标均值进行编码
        global_mean_target = y_train_temp.groupby(train_combined_df[col]).mean()
        combined_df[col_encoded_name] = combined_df[col].map(global_mean_target)
        
        # 填充新类别或缺失值，使用全局目标均值
        global_overall_mean = y_train_temp.mean()
        combined_df[col_encoded_name].fillna(global_overall_mean, inplace=True)
        print(f"已对特征 '{col}' 进行交叉验证目标编码。")
    ...
    for col in X.columns:
        if X[col].dtype == 'object': 
            try:
                # 排除原始的'model'和'brand'列（如果它们还在的话），因为它们应该被编码后的特征替代
    ...
    # 最终特征列表：移除原始的'model'和'brand'列，并包含新的目标编码特征
    final_features = [col for col in X.columns if col not in ['price', 'SaleID', 'model', 'brand']]
...
def train_and_evaluate_model(X, y, X_test, test_saleid):
    ...
    plt.xlabel('实际价格')
    plt.ylabel('预测价格')
    plt.savefig('11.10_xgb_feature_optimize_v9_price_pred_vs_true.png')
    plt.close()
    print("预测价格 vs 实际价格图已保存为 11.10_xgb_feature_optimize_v9_price_pred_vs_true.png")

    # 保存预测结果
    submission = pd.DataFrame({'SaleID': test_saleid, 'price': test_pred})
    submission.to_csv('11.10_xgb_feature_optimize_v9_predict.csv', index=False)
    print(f"预测结果已保存为 11.10_xgb_feature_optimize_v9_predict.csv")
    ...
    plt.xlabel('重要性')
    plt.ylabel('特征')
    plt.savefig('11.10_xgb_feature_optimize_v9_importance_top30.png')
    plt.close()
    print("特征重要性Top30图已保存为 11.10_xgb_feature_optimize_v9_importance_top30.png")

    print("XGBoost特征工程优化v9 完成。")

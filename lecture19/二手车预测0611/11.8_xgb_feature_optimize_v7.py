"""
11.8_xgb_feature_optimize_v7.py

基于11.7版本，主要优化了model特征的编码方式，同时进行频率编码和Label Encoding，并确保数据类型正确。
"""
import pandas as pd
import numpy as np
import matplotlib
...
def preprocess_and_feature_engineer(train_df, test_df):
    ...
    for feature in features_for_freq_encode:
        if feature in combined_df.columns:
            ...
            # Debugging: 打印频率编码后的统计信息
            print(f"--- 调试信息：'{feature}_freq_encoded' 列统计信息 ---")
            print(combined_df[f'{feature}_freq_encoded'].describe())
            print(combined_df[f'{feature}_freq_encoded'].value_counts(bins=5).sort_index())
            print("--------------------------------------------------")
        else:
            print(f"警告: 无法对特征 '{feature}' 进行频率编码，因为该列不存在。")

    # 8. 对model列进行Label Encoding（在频率编码之后）
    if 'model' in combined_df.columns:
        le = LabelEncoder()
        # 强制转换为字符串类型，以防万一
        combined_df['model_label_encoded'] = le.fit_transform(combined_df['model'].astype(str))
        print("已对特征 'model' 进行 Label Encoding 并生成 'model_label_encoded'.")
        # Debugging: 打印Label Encoding后的统计信息
        print(f"--- 调试信息：'model_label_encoded' 列统计信息 ---")
        print(combined_df['model_label_encoded'].describe())
        print(combined_df['model_label_encoded'].value_counts(bins=5).sort_index())
        print("--------------------------------------------------")
    else:
        print("警告: 无法对特征 'model' 进行 Label Encoding，因为该列不存在。")

    # Debugging: Print columns and dtypes after feature engineering
    print("\n--- 调试信息：特征工程后的列和数据类型 (合并DataFrame) ---")
    combined_df.info()
    print("--- 调试信息结束 ---")
    ...
    # 最终特征列表：确保包含新生成的编码特征，并移除原始的'model'列
    ...
    # Debugging: Print columns and dtypes before returning X and X_test
    print("\n--- 调试信息：返回前训练和测试集特征的列和数据类型 ---")
    print("X DataFrame columns and dtypes:")
    X.info()
    print("\nX_test DataFrame columns and dtypes:")
    X_test.info()
    print("--- 调试信息结束 ---")
...
def train_and_evaluate_model(X, y, X_test, test_saleid):
    ...
    plt.xlabel('实际价格')
    plt.ylabel('预测价格')
    plt.title('11.8_xgb_feature_optimize_v7_预测价格 vs 实际价格')
    plt.legend()
    plt.tight_layout()
    plt.savefig('11.8_xgb_feature_optimize_v7_price_pred_vs_true.png', dpi=150)
    plt.close()
    print('预测价格 vs 实际价格图已保存为 11.8_xgb_feature_optimize_v7_price_pred_vs_true.png')

    # 保存预测结果到CSV
    if test_saleid is not None:
        submission = pd.DataFrame({'SaleID': test_saleid, 'price': test_pred})
        submission.to_csv('11.8_xgb_feature_optimize_v7_predict.csv', index=False)
        print('预测结果已保存为 11.8_xgb_feature_optimize_v7_predict.csv')
    else:
    ...
    if len(importances) != len(feature_names):
        print("警告: 特征重要性和特征名数量不匹配，可能导致可视化错误。")
        # 尝试根据XGBoost模型的特征名获取重要性
        try:
            importances = xgb.get_booster().get_score(importance_type='weight')
            # 将字典转换为pandas Series以便排序和匹配
            importances = pd.Series(importances).reindex(feature_names).fillna(0).values
        except Exception as e:
            print(f"错误: 尝试重新获取特征重要性失败: {e}")
            return # 无法继续，提前返回

    indices = np.argsort(importances)[::-1][:30]
    
    # 过滤掉不存在于 feature_names 中的索引
    valid_indices = [idx for idx in indices if idx < len(feature_names)]
    
    if not valid_indices:
        print("警告: 没有有效的特征重要性可用于绘图。")
        return

    plt.figure(figsize=(10, 8))
    # 检查 importances[valid_indices][::-1] 是否为空
    if importances[valid_indices].size > 0:
        plt.barh(range(len(valid_indices)), importances[valid_indices][::-1], align='center')
        plt.yticks(range(len(valid_indices)), [feature_names[i] for i in valid_indices][::-1])
        plt.xlabel('特征重要性')
        plt.ylabel('特征')
        plt.title('11.8_xgb_feature_optimize_v7_XGBoost特征重要性Top{}'.format(len(valid_indices)))
        plt.tight_layout()
        plt.savefig('11.8_xgb_feature_optimize_v7_importance_top30.png', dpi=150)
        plt.close()
        print('特征重要性Top30图已保存为 11.8_xgb_feature_optimize_v7_importance_top30.png')
    else:
        print("警告: 特征重要性数据为空，无法生成Top30图。")

"""
11.17_xgb_feature_optimize_v16.py

基于11.16版本，进一步探索新的交互特征和时间相关特征，以提升模型性能。
"""
import pandas as pd
import numpy as np
import matplotlib
...
def preprocess_and_feature_engineer(train_df, test_df):
    ...
    # 在目标编码完成后，移除原始的分类列
    for col_to_drop_original in target_encode_cols:
        if col_to_drop_original in combined_df.columns:
            combined_df = combined_df.drop(columns=[col_to_drop_original])
            print(f"已从 combined_df 中移除原始列 '{col_to_drop_original}'.")
    ...
    # --- 新增交互特征和时间相关特征 ---
    # 10. kilometer * power 交互特征
    if 'kilometer' in combined_df.columns and 'power' in combined_df.columns:
        combined_df['kilometer_power_inter'] = combined_df['kilometer'] * combined_df['power']
        print("已添加特征 'kilometer_power_inter'.")
    else:
        print("警告: 无法添加 'kilometer_power_inter'，因为 'kilometer' 或 'power' 列不存在。")

    # 11. v_3 * kilometer 交互特征
    if 'v_3' in combined_df.columns and 'kilometer' in combined_df.columns:
        combined_df['v_3_kilometer_inter'] = combined_df['v_3'] * combined_df['kilometer']
        print("已添加特征 'v_3_kilometer_inter'.")
    else:
        print("警告: 无法添加 'v_3_kilometer_inter'，因为 'v_3' 或 'kilometer' 列不存在。")

    # 12. v_12 * power 交互特征
    if 'v_12' in combined_df.columns and 'power' in combined_df.columns:
        combined_df['v_12_power_inter'] = combined_df['v_12'] * combined_df['power']
        print("已添加特征 'v_12_power_inter'.")
    else:
        print("警告: 无法添加 'v_12_power_inter'，因为 'v_12' 或 'power' 列不存在。")

    # 13. age_on_sale (上市车龄) 特征
    if 'creatYear' in combined_df.columns and 'regYear' in combined_df.columns:
        combined_df['age_on_sale'] = combined_df['creatYear'] - combined_df['regYear']
        # 确保年龄非负，对异常值进行处理 (例如，设为0或中位数，这里简单设为0)
        combined_df['age_on_sale'][combined_df['age_on_sale'] < 0] = 0 
        print("已添加特征 'age_on_sale'.")
    else:
        print("警告: 无法添加 'age_on_sale'，因为 'creatYear' 或 'regYear' 列不存在。")


    # 明确定义最终特征列表 (包括原始数值型特征和所有新增的增强特征)
    # 确保不包含原始的 'brand' 和 'model'，只包含它们的目标编码版本
    # 过滤掉非数值列、is_train、price、SaleID 和 bodyType_missing
    excluded_cols = ['is_train', 'price', 'SaleID', 'bodyType_missing'] # 排除bodyType_missing
    final_features_to_select = [col for col in combined_df.columns if combined_df[col].dtype != 'object' and col not in excluded_cols]
    ...
    # 新增交互特征
    if 'kilometer_power_inter' in combined_df.columns and 'kilometer_power_inter' not in final_features_to_select:
        final_features_to_select.append('kilometer_power_inter')
    if 'v_3_kilometer_inter' in combined_df.columns and 'v_3_kilometer_inter' not in final_features_to_select:
        final_features_to_select.append('v_3_kilometer_inter')
    if 'v_12_power_inter' in combined_df.columns and 'v_12_power_inter' not in final_features_to_select:
        final_features_to_select.append('v_12_power_inter')
    if 'age_on_sale' in combined_df.columns and 'age_on_sale' not in final_features_to_select:
        final_features_to_select.append('age_on_sale')
...
def train_and_evaluate_model(X, y, X_test, test_saleid):
    ...
    plt.xlabel('实际价格')
    plt.ylabel('预测价格')
    plt.savefig('11.17_xgb_feature_optimize_v16_price_pred_vs_true.png')
    plt.close()
    print("预测价格 vs 实际价格图已保存为 11.17_xgb_feature_optimize_v16_price_pred_vs_true.png")

    # 保存预测结果
    submission = pd.DataFrame({'SaleID': test_saleid, 'price': test_pred})
    submission.to_csv('11.17_xgb_feature_optimize_v16_predict.csv', index=False)
    print(f"预测结果已保存为 11.17_xgb_feature_optimize_v16_predict.csv")
    ...
    plt.xlabel('重要性')
    plt.ylabel('特征')
    plt.savefig('11.17_xgb_feature_optimize_v16_importance_top30.png')
    plt.close()
    print("特征重要性Top30图已保存为 11.17_xgb_feature_optimize_v16_importance_top30.png")

    print("XGBoost特征工程优化v16 完成。")

...
7.8的基础上
训练集85%，验证集15%
[5999]  validation_0-mae:0.11461
XGBoost训练完成。
验证集 MAE: 503.2317
验证集 RMSE: 1200.6052

成绩：489.8
测试集预测结果已保存到 7.9_xgb_after_ablation_mae_predict.csv
特征重要性Top30图已保存为 7.9_xgb_after_ablation_mae_importance_top30.png
'''
...
def main():
    ...
    # 5. 验证集评估
    val_pred = np.expm1(xgb.predict(X_val))
    ...
    result.to_csv('7.9_xgb_after_ablation_mae_predict.csv', index=False)
    print('测试集预测结果已保存到 7.9_xgb_after_ablation_mae_predict.csv')
    ...
    plt.xlabel('特征重要性')
    plt.title('7.9_xgb_after_ablation_mae_XGBoost特征重要性Top30')
    plt.tight_layout()
    plt.savefig('7.9_xgb_after_ablation_mae_importance_top30.png', dpi=150)
    plt.close()
    print('特征重要性Top30图已保存为 7.9_xgb_after_ablation_mae_importance_top30.png')

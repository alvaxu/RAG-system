...
def main():
    ...
    # ========== 最终训练特征概览 ==========
    with open("6.1_blending_100_features.txt", "w", encoding="utf-8") as f:
        f.write("========== 最终训练特征概览 ==========" + "\n")
    ...
    print("XGBoost训练完成。")
    joblib.dump(xgb, '6.1_blending_100_xgb_model.joblib')
    print('XGBoost模型已保存到 6.1_blending_100_xgb_model.joblib')
    ...
    print("CatBoost训练完成。")
    joblib.dump(cat, '6.1_blending_100_cat_model.joblib')
    print('CatBoost模型已保存到 6.1_blending_100_cat_model.joblib')
    ...
    print("LightGBM训练完成。")
    joblib.dump(lgb, '6.1_blending_100_lgb_model.joblib')
    print('LightGBM模型已保存到 6.1_blending_100_lgb_model.joblib')
    ...
    result.to_csv('6.1_blending_100_predict.csv', index=False)
    print('Blending预测结果已保存到 6.1_blending_100_predict.csv')
    ...
    plt.xlabel('特征重要性')
    plt.title('6.1_blending_100_XGBoost特征重要性Top50')
    plt.tight_layout()
    plt.savefig('6.1_blending_100_importance_top50.png', dpi=150)
    plt.close()
    print('特征重要性Top50图已保存为 6.1_blending_100_importance_top50.png')

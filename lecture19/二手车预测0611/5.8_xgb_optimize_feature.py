'''
早停，删除一些特征，和4.2.1Best一致,特征定义也一致
模型调优后的超参数和4.2.1Best一致:
[737]   validation_0-rmse:0.22510
XGBoost模型训练结束，结束时间: Wed Jun 11 14:37:03 2025

总训练时长: 255.74秒
模型已保存到5.8_xgb_optimize_feature_model.joblib
...
模型调优后的超参数:
  ...
  random_state: 42
  n_jobs: -1
预测结果已保存到 5.8_xgb_optimize_feature_predict.csv
...
========== 绘制预测价格和实际价格对比图 ==========
预测价格与实际价格对比图已保存为 5.8_xgb_optimize_feature_price_pred_vs_true.png
特征重要性Top50图已保存为 5.8_xgb_optimize_feature_importance_top50.png
...
def main():
    ...
    # 4. 保存模型
    joblib.dump(xgb, '5.8_xgb_optimize_feature_model.joblib')
    print('模型已保存到5.8_xgb_optimize_feature_model.joblib')
    ...
    result.to_csv('5.8_xgb_optimize_feature_predict.csv', index=False)
    print('预测结果已保存到 5.8_xgb_optimize_feature_predict.csv')
    ...
    plt.tight_layout()
    plt.savefig('5.8_xgb_optimize_feature_price_pred_vs_true.png', dpi=150)
    plt.close()
    print('预测价格与实际价格对比图已保存为 5.8_xgb_optimize_feature_price_pred_vs_true.png')
    ...
    plt.xlabel('特征重要性')
    plt.title('5.8_xgb_optimize_feature_XGBoost特征重要性Top50')
    plt.tight_layout()
    plt.savefig('5.8_xgb_optimize_feature_importance_top50.png', dpi=150)
    plt.close()
    print('特征重要性Top50图已保存为 5.8_xgb_optimize_feature_importance_top50.png')

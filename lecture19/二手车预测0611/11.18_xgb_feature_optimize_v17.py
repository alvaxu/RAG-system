"""
11.18_xgb_feature_optimize_v17.py

基于11.17版本，移除导致性能下降的新增交互特征和时间相关特征，并更新所有输出文件名为11.18_。
"""
import pandas as pd
import numpy as np
import matplotlib
...
def preprocess_and_feature_engineer(train_df, test_df):
    ...
    excluded_cols = ['is_train', 'price', 'SaleID', 'bodyType_missing',
                     'kilometer_power_inter', 'v_3_kilometer_inter', 'v_12_power_inter', 'age_on_sale'] # 排除v16新增的特征
...
def train_and_evaluate_model(X, y, X_test, test_saleid):
    ...
    plt.xlabel('实际价格')
    plt.ylabel('预测价格')
    plt.savefig('11.18_xgb_feature_optimize_v17_price_pred_vs_true.png')
    plt.close()
    print("预测价格 vs 实际价格图已保存为 11.18_xgb_feature_optimize_v17_price_pred_vs_true.png")

    # 保存预测结果
    submission = pd.DataFrame({'SaleID': test_saleid, 'price': test_pred})
    submission.to_csv('11.18_xgb_feature_optimize_v17_predict.csv', index=False)
    print(f"预测结果已保存为 11.18_xgb_feature_optimize_v17_predict.csv")
    ...
    plt.xlabel('重要性')
    plt.ylabel('特征')
    plt.savefig('11.18_xgb_feature_optimize_v17_importance_top30.png')
    plt.close()
    print("特征重要性Top30图已保存为 11.18_xgb_feature_optimize_v17_importance_top30.png")

    print("XGBoost特征工程优化v17 完成。")

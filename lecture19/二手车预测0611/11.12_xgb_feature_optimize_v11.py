"""
11.12_xgb_feature_optimize_v11.py

基于11.10版本，移除regionCode的目标编码，并移除model和brand目标编码的平滑因子，回到更精简的目标编码。
"""
import pandas as pd
import numpy as np
import matplotlib
...
def preprocess_and_feature_engineer(train_df, test_df):
    ...
    # 仅保留brand和model，移除平滑因子
    target_encode_cols = ['brand', 'model'] # 移除 regionCode
    ...
    # 确保X和X_test的列顺序一致
    missing_in_test = set(X.columns) - set(X.columns) # 修改为与X的列比较，而不是X_test的列
    for col in missing_in_test:
        X_test[col] = 0 # 简单填充0
    missing_in_train = set(X.columns) - set(X.columns) # 同样修改
    for col in missing_in_train:
        X[col] = 0 # 简单填充0
    X_test = X_test[X.columns] # 确保列顺序一致

    # 最终特征列表：移除原始的'model'和'brand'列，并包含新的目标编码特征
    # regionCode不再从原始列中移除，因为它会通过11.0的LabelEncoder被包含
...
def train_and_evaluate_model(X, y, X_test, test_saleid):
    ...
    plt.xlabel('实际价格')
    plt.ylabel('预测价格')
    plt.savefig('11.12_xgb_feature_optimize_v11_price_pred_vs_true.png')
    plt.close()
    print("预测价格 vs 实际价格图已保存为 11.12_xgb_feature_optimize_v11_price_pred_vs_true.png")

    # 保存预测结果
    submission = pd.DataFrame({'SaleID': test_saleid, 'price': test_pred})
    submission.to_csv('11.12_xgb_feature_optimize_v11_predict.csv', index=False)
    print(f"预测结果已保存为 11.12_xgb_feature_optimize_v11_predict.csv")
    ...
    plt.xlabel('重要性')
    plt.ylabel('特征')
    plt.savefig('11.12_xgb_feature_optimize_v11_importance_top30.png')
    plt.close()
    print("特征重要性Top30图已保存为 11.12_xgb_feature_optimize_v11_importance_top30.png")

    print("XGBoost特征工程优化v11 完成。")

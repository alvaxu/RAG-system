...
"""
基于4.2.1的特征工程，XGBoost单模型训练，用MAE早停，7000轮，早停轮次100轮
[6999]  validation_0-mae:0.11439
XGBoost模型训练结束，结束时间: Thu Jun 12 19:34:44 2025

总训练时长: 3676.76秒
模型已保存到8.0_xgb_best.joblib
测试集预测结果已保存到8.0_xgb_best_predict.csv
验证集 MAE: 501.7029, RMSE: 1199.0296
成绩：486.6


预测价格与实际价格对比图已保存为 8.0_xgb_best_price_pred_vs_true.png.png
特征重要性Top30图已保存为 8.0_xgb_best_importance_top30.png
"""
...
def main():
    ...
    xgb = XGBRegressor(
        max_depth=10, learning_rate=0.03, n_estimators=7000, subsample=0.8, colsample_bytree=0.9,
    ...
    # 4. 保存模型
    joblib.dump(xgb, '8.0_xgb_best.joblib')
    print('模型已保存到8.0_xgb_best.joblib')
    ...
    result.to_csv('8.0_xgb_best_predict.csv', index=False)
    print('测试集预测结果已保存到8.0_xgb_best_predict.csv')
    ...
    plt.tight_layout()
    plt.savefig('8.0_xgb_best_price_pred_vs_true.png', dpi=150)
    plt.close()
    print('预测价格与实际价格对比图已保存为 8.0_xgb_best_price_pred_vs_true.png.png')
    ...
    plt.xlabel('特征重要性')
    plt.title('8.0_xgb_best 特征重要性Top30')
    plt.tight_layout()
    plt.savefig('8.0_xgb_best_importance_top30.png', dpi=150)
    plt.close()
    print('特征重要性Top30图已保存为 8.0_xgb_best_importance_top30.png')

'''
11.3_xgb_feature_optimize_v2.py
...
def preprocess_and_feature_engineer(train_df, test_df):
    ...
    # 这里不再使用LabelEncoder，因为将使用Target Encoding
    ...
    combined_df['v_0_cub'] = combined_df['v_0']**3
    ...
    # 将非字符串的类别特征转换为字符串，以便groupby能正确处理
    ...
    smoothing_factor = 100 # 平滑因子，可以根据数据情况调整
...
def train_and_evaluate_model(X, y, X_test, test_saleid):
    ...
    sample_weight[y_train_log > high_price_threshold_log] = 3.0 # 将高价样本权重设置为3倍
    ...
    plt.xlabel('实际价格')
    plt.ylabel('预测价格')
    plt.title('11.3_xgb_feature_optimize_v2_预测价格 vs 实际价格')
    plt.legend()
    plt.tight_layout()
    plt.savefig('11.3_xgb_feature_optimize_v2_price_pred_vs_true.png', dpi=150)
    plt.close()
    print('预测价格 vs 实际价格图已保存为 11.3_xgb_feature_optimize_v2_price_pred_vs_true.png')

    # 保存预测结果到CSV
    if test_saleid is not None:
        submission = pd.DataFrame({'SaleID': test_saleid, 'price': test_pred})
        submission.to_csv('11.3_xgb_feature_optimize_v2_predict.csv', index=False)
        print('预测结果已保存为 11.3_xgb_feature_optimize_v2_predict.csv')
    else:
    ...
    plt.xlabel('特征重要性')
    plt.ylabel('特征')
    plt.title('11.3_xgb_feature_optimize_v2_XGBoost特征重要性Top30')
    plt.tight_layout()
    plt.savefig('11.3_xgb_feature_optimize_v2_importance_top30.png', dpi=150)
    plt.close()
    print('特征重要性Top30图已保存为 11.3_xgb_feature_optimize_v2_importance_top30.png')
    print("XGBoost特征工程优化v2 完成。")

'''
11.5_xgb_feature_optimize_v4.py
...
def preprocess_and_feature_engineer(train_df, test_df):
    ...
    # 确保训练集和测试集具有相同的列（在Target Encoding和Frequency Encoding后会有新列）
...
def train_and_evaluate_model(X, y, X_test, test_saleid):
    ...
    plt.xlabel('实际价格')
    plt.ylabel('预测价格')
    plt.title('11.5_xgb_feature_optimize_v4_预测价格 vs 实际价格')
    plt.legend()
    plt.tight_layout()
    plt.savefig('11.5_xgb_feature_optimize_v4_price_pred_vs_true.png', dpi=150)
    plt.close()
    print('预测价格 vs 实际价格图已保存为 11.5_xgb_feature_optimize_v4_price_pred_vs_true.png')

    # 保存预测结果到CSV
    if test_saleid is not None:
        submission = pd.DataFrame({'SaleID': test_saleid, 'price': test_pred})
        submission.to_csv('11.5_xgb_feature_optimize_v4_predict.csv', index=False)
        print('预测结果已保存为 11.5_xgb_feature_optimize_v4_predict.csv')
    else:
    ...
    plt.xlabel('特征重要性')
    plt.ylabel('特征')
    plt.title('11.5_xgb_feature_optimize_v4_XGBoost特征重要性Top30')
    plt.tight_layout()
    plt.savefig('11.5_xgb_feature_optimize_v4_importance_top30.png', dpi=150)
    plt.close()
    print('特征重要性Top30图已保存为 11.5_xgb_feature_optimize_v4_importance_top30.png')
    print("XGBoost特征工程优化v4 完成。")

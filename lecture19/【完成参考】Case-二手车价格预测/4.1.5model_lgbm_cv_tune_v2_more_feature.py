...
def main():
    ...
    best_iterations = []
    for fold, (train_index, val_index) in enumerate(kf.split(X, y)):
        # ... inside the fold loop, after training ...
        best_iterations.append(lgbm.best_iteration_)

    # 计算平均最佳迭代次数
    avg_best_iteration = int(np.mean(best_iterations))
    print(f"平均最佳迭代次数: {avg_best_iteration}")

    # 4. 使用全部训练数据重新训练模型并保存
    print(f"\n使用全部数据进行最终模型训练，迭代次数: {avg_best_iteration}")
    ...
    start_time_final = time.time() # 记录最终模型训练开始时间
    final_lgbm.fit(X, np.log1p(y)) # 使用全部训练数据进行训练
    end_time_final = time.time() # 记录最终模型训练结束时间
    print(f"最终模型训练结束，时长: {end_time_final - start_time_final:.2f}秒")

    # 保存最终模型
    model_filename = f'lgbm_model_final_iter_{avg_best_iteration}.joblib'
    joblib.dump(final_lgbm, model_filename)
    print(f'最终模型已保存到 {model_filename}')

    # 5. 使用交叉验证的out-of-fold预测结果进行评估和可视化
    print(f'\n交叉验证 OOF MAE: {mean_absolute_error(y, oof_preds):.4f}, OOF RMSE: {np.sqrt(mean_squared_error(y, oof_preds)):.4f}')

    # 6. 绘制预测价格和实际价格对比图（使用out-of-fold预测）
    plt.figure(figsize=(8,6))
    plt.scatter(y, oof_preds, alpha=0.3)
    plt.plot([y.min(), y.max()], [y.min(), y.max()], 'r--', lw=2)
    plt.xlabel('实际价格')
    plt.ylabel('预测价格')
    plt.title('LGBM交叉验证 OOF预测价格 vs 实际价格')
    plt.tight_layout()
    plt.savefig('price_pred_vs_true_lgbm_cv.png', dpi=150)
    plt.close()
    print('LGBM交叉验证 OOF预测价格与实际价格对比图已保存为 price_pred_vs_true_lgbm_cv.png')

    # 7. 绘制特征重要性top20 (使用平均重要性)
    ...
    plt.xlabel('特征重要性 (平均)')
    plt.title('LGBM交叉验证 特征重要性Top20')
    plt.tight_layout()
    plt.savefig('lgbm_feature_importance_top20_cv.png', dpi=150)
    plt.close()
    print('LGBM交叉验证 特征重要性Top20图已保存为 lgbm_feature_importance_top20_cv.png')

    # 8. 使用最终模型对测试集进行预测并保存结果
    ...
    result_filename = f'used_car_sample_submit_lgbm_final_iter_{avg_best_iteration}.csv'

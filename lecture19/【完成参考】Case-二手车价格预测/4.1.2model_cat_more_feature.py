...
def main():
    ...
    # 其余类别特征不做LabelEncoder，直接保留原始字符串
    # 匿名特征及PCA降维、数值归一化同前
    ...
    # 在构建最终特征列表和划分数据集前，统一转换所有CatBoost类别特征为string
    ...
    # CatBoost支持直接传递类别特征索引
    cat_features_index = [X.columns.get_loc(col) for col in catboost_cat_features if col in X.columns]
    ...
    print(f"CatBoost模型训练开始，开始时间: {time.ctime(start_time)}\n") # 打印开始时间
    cat_model = CatBoostRegressor(
        ...
        verbose=100  # 每100步输出一次进度
    )
    cat_model.fit(X_train, np.log1p(y_train), eval_set=(X_val, np.log1p(y_val)), early_stopping_rounds=50)
    end_time = time.time() # 记录结束时间
    print(f"CatBoost模型训练结束，结束时间: {time.ctime(end_time)}\n") # 打印结束时间
    print(f"总训练时长: {end_time - start_time:.2f}秒") # 打印总时长
    # 4. 保存模型
    cat_model.save_model('catboost_model_final_5000.cbm')
    print('模型已保存到 catboost_model_final_5000.cbm')

    # 5. 预测
    y_val_pred_log = cat_model.predict(X_val)
    y_val_pred = np.expm1(y_val_pred_log)
    test_pred_log = cat_model.predict(X_test)
    ...
    result.to_csv('used_car_sample_submit_catboost_final_5000.csv', index=False)
    print('预测结果已保存到 used_car_sample_submit_catboost_final_5000.csv')
    ...
    plt.tight_layout()
    plt.savefig('price_pred_vs_true_catboost_5000.png', dpi=150)
    plt.close()
    print('预测价格与实际价格对比图已保存为 price_pred_vs_true_catboost_5000.png')
    ...
    plt.xlabel('特征重要性')
    plt.title('CatBoost特征重要性Top20')
    plt.tight_layout()
    plt.savefig('catboost_feature_importance_top20_5000.png', dpi=150)
    plt.close()
    print('特征重要性Top20图已保存为 catboost_feature_importance_top20_5000.png')

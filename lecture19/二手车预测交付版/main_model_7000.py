...
基于最优稳定版本，用于二手车价格预测的最终模型代码。
[6999]  validation_0-mae:0.11403
XGBoost模型重新训练完成。
重新训练模型在验证集上的 MAE (xgb.best_score): 0.1140
验证集 MAE: 504.9433
验证集 RMSE: 1217.8002
成绩：486.9572
"""
...
def preprocess_and_feature_engineer(train_df, test_df):
    ...
    # --- 移除V20中引入的导致性能下降的特征 ---
    if 'v_12_power_inter' in combined_df.columns:
        combined_df = combined_df.drop(columns=['v_12_power_inter'])
        print("已移除特征 'v_12_power_inter'.")
    if 'v_3_kilometer_inter' in combined_df.columns:
        combined_df = combined_df.drop(columns=['v_3_kilometer_inter'])
        print("已移除特征 'v_3_kilometer_inter'.")
    if 'car_age_power_ratio' in combined_df.columns:
        combined_df = combined_df.drop(columns=['car_age_power_ratio'])
        print("已移除特征 'car_age_power_ratio'.")
...
def train_and_evaluate_model(X, y, X_test, test_saleid):
    ...
    plt.xlabel('实际价格')
    plt.ylabel('预测价格')
    plt.savefig('main_model_7000_price_pred_vs_true.png')
    plt.close()
    print("预测价格 vs 实际价格图已保存为 main_model_7000_price_pred_vs_true.png")

    # 保存预测结果
    submission = pd.DataFrame({'SaleID': test_saleid, 'price': test_pred})
    submission.to_csv('main_model_7000_predict.csv', index=False)
    print(f"预测结果已保存为 main_model_7000_predict.csv")

    # 保存验证集预测
    val_pred_df = pd.DataFrame({'SaleID': X_val['SaleID'] if 'SaleID' in X_val.columns else np.arange(len(X_val)), 'price': val_pred})
    val_pred_df.to_csv('main_model_7000_val_pred.csv', index=False)
    ...
    plt.xlabel('重要性')
    plt.ylabel('特征')
    plt.savefig('main_model_7000_importance_top30.png')
    plt.close()
    print("特征重要性Top30图已保存为 main_model_7000_importance_top30.png")

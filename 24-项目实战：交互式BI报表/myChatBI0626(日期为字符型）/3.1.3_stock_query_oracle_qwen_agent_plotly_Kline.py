...
class ExcSQLTool(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        try:
            ...
            df['trade_date_str'] = pd.to_datetime(df['trade_date'], format='%Y%m%d').dt.strftime('%Y-%m-%d')
            if need_kline:
                ...
                if kline_fields.issubset(df.columns):
                    ...
                    fig.update_layout(
                        ...
                        xaxis=dict(
                            type='category',
                            tickmode='auto',   # 自动疏密
                            nticks=10,         # 最多显示10个主刻度（可根据实际调整）
                            tickangle=45,      # 旋转角度
                            tickfont=dict(family='SimHei,Microsoft YaHei,Arial Unicode MS', size=12),
                        )
                    )
                    # 保存为静态图片

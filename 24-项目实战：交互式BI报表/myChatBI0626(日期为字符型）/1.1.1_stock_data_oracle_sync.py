...
# SQL建表文件路径
SQL_FILE = '1.1.0_oracle_create_stock_data_table.sql'
...
def get_latest_trade_date(conn, ts_code):
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT MAX(trade_date) FROM stock_data WHERE ts_code = :ts_code
    """, ts_code=ts_code)
    result = cursor.fetchone()[0]
    cursor.close()
    # 由于trade_date已为字符串类型，直接返回
    return result
...
def insert_stock_data(conn, stock_name, ts_code, df):
    ...
    # 保持trade_date为字符串类型（yyyyMMdd），不再转为datetime
...
def main():
    ...
    try:
        # 1. 检查表是否存在
        if not table_exists(conn, 'stock_data'):
            print('表stock_data不存在，正在创建...')

'''
程序说明：
## 1. 本程序在4.1.3_stock_predict_lstm.py基础上，集成了全部数据处理、特征工程、递推链条、可视化等功能，并优化了LSTM模型结构和训练策略。
## 2. 主要改进：采用2层LSTM（每层128单元）、Dropout、BatchNormalization、EarlyStopping、ModelCheckpoint，训练过程详细输出loss和预测分布。
## 3. 其它功能、注释、输出、可视化等全部保留并兼容。
'''
...
def train_model(model, X_train, y_train, model_path, save_dir, stock_name, window):
    """
    :function: 训练LSTM模型，并保存最佳模型和loss曲线
    :param model: LSTM模型
    :param X_train: 训练特征
    :param y_train: 训练标签
    ...
    print('训练集预测分布:')
    print('均值:', np.mean(y_pred))
    print('方差:', np.var(y_pred))
    print('最大:', np.max(y_pred))
    print('最小:', np.min(y_pred))
    return history

# ===================== 递推与可视化等其它函数（复用4.1.3） =====================
# ... 递推链条、可视化、主流程等内容请参考4.1.3，全部整合进来 ...
...
def main():
    ...
    df = add_derived_features(df)  # 恢复部分衍生特征
    feature_cols = [
        'open','high','low','close','pre_close','change','pct_chg','vol','amount',
        'weekday','month_period','is_month_end','is_holiday','is_trade_day','is_post_holiday_trade_day',
        'ma5','ma10','close_ma5_diff','close_ma10_diff','pct_chg_mean5','pct_chg_std5'
    ]
    ...
    save_dir = '4.1.4_lstm_pred_image_show'

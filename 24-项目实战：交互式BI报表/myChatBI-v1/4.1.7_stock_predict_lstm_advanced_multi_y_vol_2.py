'''
程序说明：
## 1. 本程序在4.1.6_stock_predict_lstm_advanced_multi_y_vol.py基础上，升级为多目标预测（open、close、high、low、vol），递推链条每步预测5个特征。
## 2. 主要改进：LSTM输出层为Dense(5)，损失函数为MSE，递推时窗口open/close/high/low/vol均用模型预测值，pre_close用上一步的close。
## 3. 输出目录为4.1.6_lstm_pred_image_show，所有输出、可视化、特征导出等与4.1.6一致。
...
def main():
    ...
    save_dir = '4.1.7_lstm_pred_image_show'

'''
程序说明：
## 1. 本程序为3.1.4版股票历史数据查询助手，基于Qwen_agent的Assistant，支持终端TUI和Web GUI两种模式。
## 2. 在3.1.3基础上，新增ARIMA预测功能：工具arima_stock支持对指定股票未来N天收盘价进行预测，预测结果以表格和图片形式返回。
## 3. K线图绘制部分用plotly，其他可视化为matplotlib，输出图片目录、文件名规则与3.1.3一致。
'''
...
class ArimaStockTool(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        try:
            ...
            plt.tight_layout()
            # 保存图片
            pred_dir = os.path.join(os.path.dirname(__file__), '3.1.4_arima_image_show')
...
class ExcSQLTool(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        try:
            ...
            # K线图输出目录
            kline_dir = os.path.join(os.path.dirname(__file__), '3.1.4_image_show')
            # 折线/对比图输出目录
            plot_dir = os.path.join(os.path.dirname(__file__), '3.1.4_image_show')

'''
程序说明：
## 1. 本程序为3.1.7版股票历史数据查询助手，基于Qwen_agent的Assistant，支持终端TUI和Web GUI两种模式。
## 2. 在3.1.6基础上，新增prophet_predict工具，支持对指定股票未来n天收盘价进行预测，Prophet参数和节假日处理参考3.2.5_prophet_model(122).py。
## 3. 其余功能（K线、ARIMA、BOLL、周期分解等）与3.1.6一致。
'''
...
class ProphetPredictTool(BaseTool):
    ...
    :param start_date: 训练数据起始日期，格式YYYY-MM-DD，选填，默认近一年（今天-365天）
    :param end_date: 训练数据结束日期，格式YYYY-MM-DD，选填，默认今天
    :return: 预测结果表格和趋势图图片
    """
    description = '对指定股票未来n天收盘价进行Prophet预测，n为未来预测天数，训练数据默认为近一年，返回表格和图片。参数含义与实际用途已明确区分。'
    parameters = [
        {'name': 'ts_code', 'type': 'string', 'description': '股票代码', 'required': True},
        {'name': 'n', 'type': 'integer', 'description': '未来预测天数', 'required': True},
        {'name': 'start_date', 'type': 'string', 'description': '训练数据起始日期，格式YYYY-MM-DD，选填，默认近一年', 'required': False},
        {'name': 'end_date', 'type': 'string', 'description': '训练数据结束日期，格式YYYY-MM-DD，选填，默认今天', 'required': False}
    ]
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        try:
            ...
            # ========== 仅考虑长假（连续>=3天的法定假期） ===========
            all_days = pd.date_range(start=start_date, end=end_date + timedelta(days=n), freq='D')
            ...
            # ========== Prophet建模，参数参考3.2.5_prophet_model(122).py ===========
            m = Prophet(
            ...
            plt.plot(df['ds'], df['y'], label='历史收盘价')
            plt.plot(pred_df['ds'], pred_df['yhat'], 'r--', label='预测收盘价')
            ...
            plt.tight_layout()
            prophet_pred_dir = os.path.join(os.path.dirname(__file__), '3.1.7_prophet_pred_image_show')
            ...
            return f"{pred_md}\n\n{img_md}\n\n**注：n为未来预测天数，训练数据区间默认为近一年，除非用户明确指定。**"
...
class ProphetAnalysisTool(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        try:
            ...
            forecast = m.predict(future)
            # ========== 可视化 ===========
            prophet_dir = os.path.join(os.path.dirname(__file__), '3.1.7_prophet_image_show')
...
class BollDetectionTool(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        try:
            ...
            plt.tight_layout()
            # 保存图片
            boll_dir = os.path.join(os.path.dirname(__file__), '3.1.7_boll_image_show')
...
class ArimaStockTool(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        try:
            ...
            plt.tight_layout()
            # 保存图片
            pred_dir = os.path.join(os.path.dirname(__file__), '3.1.7_arima_image_show')
...
class ExcSQLTool(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        try:
            ...
            # K线图输出目录
            kline_dir = os.path.join(os.path.dirname(__file__), '3.1.7_image_show')
            # 折线/对比图输出目录
            plot_dir = os.path.join(os.path.dirname(__file__), '3.1.7_image_show')

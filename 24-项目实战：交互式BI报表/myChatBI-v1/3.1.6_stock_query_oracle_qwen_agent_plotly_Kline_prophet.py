'''
程序说明：
## 1. 本程序为3.1.6版股票历史数据查询助手，基于Qwen_agent的Assistant，支持终端TUI和Web GUI两种模式。
## 2. 在3.1.5基础上，新增Prophet周期性分析功能：工具prophet_analysis支持对指定股票过去一年（或自定义区间）进行周期性分析，自动分析趋势（trend）、周（weekly）、月（monthly）、年（yearly）等成分，并用prophet进行可视化，输出表格和图片。
## 3. 其余功能（K线、ARIMA、BOLL等）与3.1.5一致，所有输出图片目录、文件名规则与3.1.5一致。
'''
...
import chinese_calendar  # 新增节假日分析
...
class ProphetAnalysisTool(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        try:
            ...
            # ========== 可视化 ===========
            prophet_dir = os.path.join(os.path.dirname(__file__), '3.1.6_prophet_image_show')
...
class BollDetectionTool(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        try:
            ...
            plt.tight_layout()
            # 保存图片
            boll_dir = os.path.join(os.path.dirname(__file__), '3.1.6_boll_image_show')
...
class ArimaStockTool(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        try:
            ...
            plt.tight_layout()
            # 保存图片
            pred_dir = os.path.join(os.path.dirname(__file__), '3.1.6_arima_image_show')
...
class ExcSQLTool(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        try:
            ...
            # K线图输出目录
            kline_dir = os.path.join(os.path.dirname(__file__), '3.1.6_image_show')
            # 折线/对比图输出目录
            plot_dir = os.path.join(os.path.dirname(__file__), '3.1.6_image_show')

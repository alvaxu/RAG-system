'''
程序说明：
## 1. 本程序为5.0.1版股票历史数据智能分析与预测系统的TUI（命令行菜单式交互）主入口。
## 2. 在5.0.0基础上，增加了数据为空时的防御性判断，避免因无数据导致的IndexError，用户体验更友好。
'''

import pandas as pd
import traceback
from v500_config import STOCKS, ORACLE_CONN_STR, IMAGE_OUTPUT_DIR, LSTM_DEFAULTS, PRED_DAYS, DEFAULT_STOCK_NAME, DEFAULT_START_DATE, DEFAULT_END_DATE
...
from v501_lstm_predict import lstm_predict
...
# ========== TUI主流程 ==========
def main():
    print("欢迎使用5.0.1版股票历史数据智能分析与预测系统！\n")
    while True:
        print("""
请选择功能：
1. 特征工程
...
""")
        ...
        # 默认股票、日期均引用配置文件
        stock_name = input(f"请输入股票名称（默认:{DEFAULT_STOCK_NAME}）：").strip() or DEFAULT_STOCK_NAME
        ts_code = STOCKS.get(stock_name, stock_name)
        start_date = input(f"请输入开始日期（YYYY-MM-DD，默认:{DEFAULT_START_DATE}）：").strip() or DEFAULT_START_DATE
        end_date = input(f"请输入结束日期（YYYY-MM-DD，默认:{DEFAULT_END_DATE}）：").strip() or DEFAULT_END_DATE
        try:
            df = get_stock_data(ts_code, start_date, end_date)
            print(f"【数据读取】成功，样本数：{len(df)}，字段：{list(df.columns)}")
            if df.empty:
                print("【警告】当前条件下未查询到任何数据，请检查股票代码和日期区间。")
                continue
        ...
        try:
            if choice == '1':
                ...
                lstm_result = lstm_predict(df, window=int(window), pred_days=int(pred_days), epochs=int(epochs), batch_size=int(batch_size), stock_name=stock_name, ts_code=ts_code)
                if lstm_result['future_pred'] is None:
                    print("【LSTM预测失败】：", lstm_result.get('error', '未知错误'))
                else:
